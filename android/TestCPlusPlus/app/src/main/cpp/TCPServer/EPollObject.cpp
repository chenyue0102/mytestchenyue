...
bool EPollObject::init()
{
	...
	do
	{
		...
		m_bExit = false;
		if ((errCode = pipe(m_pipe)) < 0){
			LOG(LOG_ERR, "EPollObject::init init pipe failed");
			assert(false);
			break;
		}
...
bool EPollObject::destroy()
{
	...
	do{
		...
		if (0 != m_pipe[PIPE_WRITE_INDEX])
		{
			if (write(m_pipe[PIPE_WRITE_INDEX], "0", 1) < 0)
			{
				assert(false);
				LOG(LOG_ERR, "EPollObject::destroy write pipe failed");
			}
		}
		...
		std::lock_guard<std::mutex> lk(m_mutex);
		if (0 != m_pipe[PIPE_READ_INDEX])
		{
			close(m_pipe[PIPE_READ_INDEX]);
			close(m_pipe[PIPE_WRITE_INDEX]);
			m_pipe[PIPE_READ_INDEX] = m_pipe[PIPE_WRITE_INDEX] = 0;
		}
		if (0 != m_epollfd)
		{
			close(m_epollfd);
...
{
	const int MaxEvents = std::min(32, epollSize);
