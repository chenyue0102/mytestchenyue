...
// delay
//
void delay_make(sf_rv_delay_st *delay, int size){
	delay->pos = 0;
	delay->size = clampi(size, 1, SF_REVERB_DS);
	memset(delay->buf, 0, sizeof(float) * delay->size);
}

float delay_step(sf_rv_delay_st *delay, float v){
...
// ..etc
float delay_get(sf_rv_delay_st *delay, int offset){
...
float delay_getlast(sf_rv_delay_st *delay){
	return delay->buf[delay->pos];
}
...
// iir1
//
void iir1_makeLPF(sf_rv_iir1_st *iir1, int rate, float freq){
...
float iir1_step(sf_rv_iir1_st *iir1, float v){
...
// biquad
//
void biquad_makeLPF(sf_rv_biquad_st *biquad, int rate, float freq, float bw){
	freq = clampf(freq, 0, rate / 2);
...
void biquad_makeLPFQ(sf_rv_biquad_st *biquad, int rate, float freq, float bw){
	freq = clampf(freq, 0, rate / 2);
	float omega = 2.0f * (float)M_PI * freq / (float)rate;
	float cs = cosf(omega);
...
void biquad_makeAPF(sf_rv_biquad_st *biquad, int rate, float freq, float bw){
	freq = clampf(freq, 0, rate / 2);
	float omega = 2.0f * (float)M_PI * freq / (float)rate;
	float sn = sinf(omega);
	float alpha = sn * sinhf((float)M_LN2 * 0.5f * bw * omega / sn);
...
float biquad_step(sf_rv_biquad_st *biquad, float v){
...
// earlyref
//
void earlyref_make(sf_rv_earlyref_st *earlyref, int rate, float factor, float width){
...
sf_sample_st earlyref_step(sf_rv_earlyref_st *earlyref, sf_sample_st input){
...
// oversample
//
void oversample_make(sf_rv_oversample_st *oversample, int factor){
...
void oversample_stepup(sf_rv_oversample_st *oversample, float input, float *output){
...
float oversample_stepdown(sf_rv_oversample_st *oversample, float *input){
	if (oversample->factor == 1)
		return input[0];
...
// dccut
//
void dccut_make(sf_rv_dccut_st *dccut, int rate, float freq){
	freq = clampf(freq, 0, rate / 2);
...
float dccut_step(sf_rv_dccut_st *dccut, float v){
...
// noise
//
void noise_make(sf_rv_noise_st *noise){
	noise->pos = SF_REVERB_NS;
}

float noise_step(sf_rv_noise_st *noise){
...
// lfo
//
void lfo_make(sf_rv_lfo_st *lfo, int rate, float freq){
	lfo->count = 0;
	lfo->re = 1.0f;
	lfo->im = 0.0f;
...
float lfo_step(sf_rv_lfo_st *lfo){
	float v = lfo->im;
...
// allpass
//
void allpass_make(sf_rv_allpass_st *allpass, int size, float feedback, float decay){
	allpass->pos = 0;
...
float allpass_step(sf_rv_allpass_st *allpass, float v){
...
// allpass2
//
void allpass2_make(sf_rv_allpass2_st *allpass2, int size1, int size2, float feedback1,
...
float allpass2_step(sf_rv_allpass2_st *allpass2, float v){
...
float allpass2_get1(sf_rv_allpass2_st *allpass2, int offset){
...
float allpass2_get2(sf_rv_allpass2_st *allpass2, int offset){
...
// allpass3
//
void allpass3_make(sf_rv_allpass3_st *allpass3, int size1, int msize1, int size2,
...
float allpass3_step(sf_rv_allpass3_st *allpass3, float v, float mod){
...
float allpass3_get1(sf_rv_allpass3_st *allpass3, int offset){
...
float allpass3_get2(sf_rv_allpass3_st *allpass3, int offset){
...
float allpass3_get3(sf_rv_allpass3_st *allpass3, int offset){
...
// allpassm
//
void allpassm_make(sf_rv_allpassm_st *allpassm, int size, int msize, float feedback,
	float decay){
...
float allpassm_step(sf_rv_allpassm_st *allpassm, float v, float mod, float fbmod){
...
// comb
//
void comb_make(sf_rv_comb_st *comb, int size){
	comb->pos = 0;
	comb->size = clampi(size, 1, SF_REVERB_CS);
	memset(comb->buf, 0, sizeof(float) * comb->size);
}

float comb_step(sf_rv_comb_st *comb, float v, float feedback){
