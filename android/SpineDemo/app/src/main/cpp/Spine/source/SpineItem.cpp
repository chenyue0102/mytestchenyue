...
bool SpineItem::create(AimyTextureLoader *textureLoader) {
	releaseSkeletonRelatedData();
	m_isLoading = true;
	...
	m_atlas.reset(new spine::Atlas(m_atlasFile.c_str(), mTextureLoader));
...
void SpineItem::renderToCache(RenderCmdsCache *render)
{
    ...
    for (const auto& batch: m_batches) {
        ...
        assert(nullptr != mTextureLoader);
        m_renderCache->drawTriangles(
                mTextureLoader->getGLTexture(batch.texture),
                    batch.vertices,
                    batch.triangles,
					m_blendColor,
                    m_blendColorChannel,
        ...
        if(m_debugBones || m_debugSlots || m_debugMesh) {
            ...
            if(m_debugBones) {
                ...
                for(size_t i = 0, n = m_skeleton->getBones().size(); i < n; i++) {
                    m_skeleton->updateWorldTransform();
