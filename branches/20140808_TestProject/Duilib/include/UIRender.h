...
public:
	...
	//使用stbi函数加载图片，效率较高，对于gif图片的支持没研究
	static TImageInfo* LoadImageByStbi(const BYTE* pData, DWORD dwSize, DWORD dwMask);
	//使用gid+加载图片，对于gif图片能够支持，但是效率低
	static TImageInfo* LoadImageByGdiPlus(const BYTE* pData, DWORD dwSize, DWORD dwMask);
    ...
    static bool DrawImageString(HDC hDC, CPaintManagerUI* pManager, const RECT& rcItem, const RECT& rcPaint, 
        LPCTSTR pStrImage, LPCTSTR pStrModify = NULL, DWORD dwFrameIndex = 0, const TImageInfo **ppImageInfo = NULL);
