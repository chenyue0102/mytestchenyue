...
CStdString CSkinResource::GetSkinPathName() const
{
	if (!m_strSkinFilePath.IsEmpty()) {
		if (m_strSkinFilePath.ReverseFind(_T('\\') != -1)) {
			return m_strSkinFilePath.Left(m_strSkinFilePath.ReverseFind(_T('\\')));
		}
	}
	return _T("");
}

CStdString CSkinResource::GetSkinFileName() const
{
	if (!m_strSkinFilePath.IsEmpty()) {
		if (m_strSkinFilePath.ReverseFind(_T('\\')) == -1) {
			return m_strSkinFilePath;
		} else {
			return m_strSkinFilePath.Right(m_strSkinFilePath.GetLength() - m_strSkinFilePath.ReverseFind(_T('\\')) - 1);
		}
	}
	return _T("");
}
