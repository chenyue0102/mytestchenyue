...
LRESULT CComboWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    if( uMsg == WM_CREATE ) {
		...
		m_pLayout->SetBorderColor(0xFFBBBBBB);
		...
		// Mouse wheeling on the Combo item will not notify its container
		// to scroll, but will notify the its list owner (CComboUI). So we
		// need to scroll manually here.
		if (zDelta < 0)
			m_pLayout->LineDown();
		else
			m_pLayout->LineUp();
...
void CComboUI::SetNormalImage(LPCTSTR pStrImage)
{
	...
	StopImagePlayTimerAndCleanFrameInfo(FRAME_TYPE_Combo_NormalImage);//如果定时器存在,停止这个图片的定时器
    Invalidate();
}
...
void CComboUI::SetHotImage(LPCTSTR pStrImage)
{
	...
	StopImagePlayTimerAndCleanFrameInfo(FRAME_TYPE_Combo_HotImage);//如果定时器存在,停止这个图片的定时器
    Invalidate();
}
...
void CComboUI::SetPushedImage(LPCTSTR pStrImage)
{
	...
	StopImagePlayTimerAndCleanFrameInfo(FRAME_TYPE_Combo_PushedImage);//如果定时器存在,停止这个图片的定时器
    Invalidate();
}
...
void CComboUI::SetFocusedImage(LPCTSTR pStrImage)
{
	...
	StopImagePlayTimerAndCleanFrameInfo(FRAME_TYPE_Combo_FocusedImage);//如果定时器存在,停止这个图片的定时器
    Invalidate();
}
...
void CComboUI::SetDisabledImage(LPCTSTR pStrImage)
{
	...
	StopImagePlayTimerAndCleanFrameInfo(FRAME_TYPE_Combo_DisabledImage);//如果定时器存在,停止这个图片的定时器
    Invalidate();
}
...
void CComboUI::PaintStatusImage(HDC hDC)
{
    ...
    if( (m_uButtonState & UISTATE_DISABLED) != 0 ) {
        if( !m_sDisabledImage.IsEmpty() ) {
            if( !DrawImage(hDC, (LPCTSTR)m_sDisabledImage, NULL, FRAME_TYPE_Combo_DisabledImage) ) m_sDisabledImage.Empty();
            else return;
        }
    }
    else if( (m_uButtonState & UISTATE_PUSHED) != 0 ) {
        if( !m_sPushedImage.IsEmpty() ) {
            if( !DrawImage(hDC, (LPCTSTR)m_sPushedImage, NULL, FRAME_TYPE_Combo_PushedImage) ) m_sPushedImage.Empty();
            else return;
        }
    }
    else if( (m_uButtonState & UISTATE_HOT) != 0 ) {
        if( !m_sHotImage.IsEmpty() ) {
            if( !DrawImage(hDC, (LPCTSTR)m_sHotImage, NULL, FRAME_TYPE_Combo_HotImage) ) m_sHotImage.Empty();
            else return;
        }
    }
    else if( (m_uButtonState & UISTATE_FOCUSED) != 0 ) {
        if( !m_sFocusedImage.IsEmpty() ) {
            if( !DrawImage(hDC, (LPCTSTR)m_sFocusedImage, NULL, FRAME_TYPE_Combo_FocusedImage) ) m_sFocusedImage.Empty();
            else return;
        }
    }

    if( !m_sNormalImage.IsEmpty() ) {
        if( !DrawImage(hDC, (LPCTSTR)m_sNormalImage, NULL, FRAME_TYPE_Combo_NormalImage) ) m_sNormalImage.Empty();
        else return;
    }
}
