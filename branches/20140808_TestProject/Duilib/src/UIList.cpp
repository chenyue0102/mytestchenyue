...
UINT CListUI::GetControlFlags() const
{
	...
	return __super::GetControlFlags();//防止显示界面后，就把焦点抢了过去
}
...
void CListUI::SetItemBkImage(LPCTSTR pStrImage)
{
	...
	StopImagePlayTimerAndCleanFrameInfo(FRAME_TYPE_ListInfo_BkImage);//如果定时器存在,停止这个图片的定时器
    Invalidate();
}
...
void CListUI::SetSelectedItemImage(LPCTSTR pStrImage)
{
	...
	StopImagePlayTimerAndCleanFrameInfo(FRAME_TYPE_ListInfo_SelectedImage);//如果定时器存在,停止这个图片的定时器
    Invalidate();
}
...
void CListUI::SetHotItemImage(LPCTSTR pStrImage)
{
	...
	StopImagePlayTimerAndCleanFrameInfo(FRAME_TYPE_ListInfo_HotImage);//如果定时器存在,停止这个图片的定时器
    Invalidate();
}
...
void CListUI::SetDisabledItemImage(LPCTSTR pStrImage)
{
	...
	StopImagePlayTimerAndCleanFrameInfo(FRAME_TYPE_ListInfo_DisabledImage);//如果定时器存在,停止这个图片的定时器
    Invalidate();
}
...
void CListBodyUI::SetPos(RECT rc)
{
    ...
    for( int it1 = 0; it1 < m_items.GetSize(); it1++ ) {
        ...
        if( !pControl->IsVisible() ) continue;
        if( pControl->IsFloat() ) continue;
		SIZE szEst = { szAvailable.cx, CY_INFINITE };
		SIZE sz = pControl->EstimateSize(szEst);
        if( sz.cy == 0 ) {
            nAdjustables++;
        }
        else {
    ...
    for( int it2 = 0; it2 < m_items.GetSize(); it2++ ) {
		...
		SIZE szEst = { szRemaining.cx, CY_INFINITE };
		SIZE sz = pControl->EstimateSize(szEst);
        if( sz.cy == 0 ) {
            iAdjustable++;
            sz.cy = cyExpand;
...
void CListHeaderItemUI::SetNormalImage(LPCTSTR pStrImage)
{
	...
	StopImagePlayTimerAndCleanFrameInfo(FRAME_TYPE_ListHeaderItem_NormalImage);//如果定时器存在,停止这个图片的定时器
    Invalidate();
}
...
void CListHeaderItemUI::SetHotImage(LPCTSTR pStrImage)
{
	...
	StopImagePlayTimerAndCleanFrameInfo(FRAME_TYPE_ListHeaderItem_HotImage);//如果定时器存在,停止这个图片的定时器
    Invalidate();
}
...
void CListHeaderItemUI::SetPushedImage(LPCTSTR pStrImage)
{
	...
	StopImagePlayTimerAndCleanFrameInfo(FRAME_TYPE_ListHeaderItem_PushedImage);//如果定时器存在,停止这个图片的定时器
    Invalidate();
}
...
void CListHeaderItemUI::SetFocusedImage(LPCTSTR pStrImage)
{
	...
	StopImagePlayTimerAndCleanFrameInfo(FRAME_TYPE_ListHeaderItem_FocusedImage);//如果定时器存在,停止这个图片的定时器
    Invalidate();
}
...
void CListHeaderItemUI::SetSepImage(LPCTSTR pStrImage)
{
	...
	StopImagePlayTimerAndCleanFrameInfo(FRAME_TYPE_ListHeaderItem_SepImage);//如果定时器存在,停止这个图片的定时器
    Invalidate();
}
...
void CListHeaderItemUI::PaintStatusImage(HDC hDC)
{
    ...
    if( (m_uButtonState & UISTATE_PUSHED) != 0 ) {
        if( m_sPushedImage.IsEmpty() && !m_sNormalImage.IsEmpty() ) DrawImage(hDC, (LPCTSTR)m_sNormalImage, NULL, FRAME_TYPE_ListHeaderItem_NormalImage);
        if( !DrawImage(hDC, (LPCTSTR)m_sPushedImage, NULL, FRAME_TYPE_ListHeaderItem_PushedImage) ) m_sPushedImage.Empty();
    }
    else if( (m_uButtonState & UISTATE_HOT) != 0 ) {
        if( m_sHotImage.IsEmpty() && !m_sNormalImage.IsEmpty() ) DrawImage(hDC, (LPCTSTR)m_sNormalImage, NULL, FRAME_TYPE_ListHeaderItem_NormalImage);
        if( !DrawImage(hDC, (LPCTSTR)m_sHotImage, NULL, FRAME_TYPE_ListHeaderItem_HotImage) ) m_sHotImage.Empty();
    }
    else if( (m_uButtonState & UISTATE_FOCUSED) != 0 ) {
        if( m_sFocusedImage.IsEmpty() && !m_sNormalImage.IsEmpty() ) DrawImage(hDC, (LPCTSTR)m_sNormalImage, NULL, FRAME_TYPE_ListHeaderItem_NormalImage);
        if( !DrawImage(hDC, (LPCTSTR)m_sFocusedImage, NULL, FRAME_TYPE_ListHeaderItem_FocusedImage) ) m_sFocusedImage.Empty();
    }
    else {
        if( !m_sNormalImage.IsEmpty() ) {
            if( !DrawImage(hDC, (LPCTSTR)m_sNormalImage, NULL, FRAME_TYPE_ListHeaderItem_NormalImage) ) m_sNormalImage.Empty();
        }
    }
    ...
    if( !m_sSepImage.IsEmpty() ) {
        ...
        if( !DrawImage(hDC, (LPCTSTR)m_sSepImage, (LPCTSTR)m_sSepImageModify, FRAME_TYPE_ListHeaderItem_SepImage) ) m_sSepImage.Empty();
    }
}
...
void CListElementUI::DrawItemBk(HDC hDC, const RECT& rcItem)
{
    ...
    if( !IsEnabled() ) {
        if( !pInfo->sDisabledImage.IsEmpty() ) {
            if( !DrawImage(hDC, (LPCTSTR)pInfo->sDisabledImage, NULL, FRAME_TYPE_ListInfo_DisabledImage) ) pInfo->sDisabledImage.Empty();
            else return;
        }
    }
    if( IsSelected() ) {
        if( !pInfo->sSelectedImage.IsEmpty() ) {
            if( !DrawImage(hDC, (LPCTSTR)pInfo->sSelectedImage, NULL, FRAME_TYPE_ListInfo_SelectedImage) ) pInfo->sSelectedImage.Empty();
            else return;
        }
    }
    if( (m_uButtonState & UISTATE_HOT) != 0 ) {
        if( !pInfo->sHotImage.IsEmpty() ) {
            if( !DrawImage(hDC, (LPCTSTR)pInfo->sHotImage, NULL, FRAME_TYPE_ListInfo_HotImage) ) pInfo->sHotImage.Empty();
            else return;
        }
    }
    ...
    if( m_sBkImage.IsEmpty() ) {
        if( !pInfo->sBkImage.IsEmpty() ) {
            if( !DrawImage(hDC, (LPCTSTR)pInfo->sBkImage, NULL, FRAME_TYPE_ListInfo_BkImage) ) pInfo->sBkImage.Empty();
            else return;
        }
    }
