        ...
        if( _tcscmp(pstrClass, _T("Window")) == 0 ) {
            if( pManager->GetPaintWindow()) {
                ...
                for( int i = 0; i < nAttributes; i++ ) {
					...
					else if (_tcscmp(pstrName, _T("resizewindow")) == 0)
					{
						bool bEnableResizeWindow = (_tcscmp(pstrValue, _T("true")) == 0);
						pManager->SetEnableResizeWindow(bEnableResizeWindow);
					}
                }
            }
        }
    ...
    for( CMarkupNode node = pRoot->GetChild() ; node.IsValid(); node = node.GetSibling() ) {
        ...
        if( _tcscmp(pstrClass, _T("Include")) == 0 ) {
            ...
            case 9:
				...
				else if (_tcscmp(pstrClass, _T("Accordion")) == 0)			pControl = new CAccordionUI;
                break;
            case 10:
            ...
            case 15:
				...
				else if (_tcscmp(pstrClass, _T("AccordionHeader")) == 0)	pControl = new CAccordionHeaderUI;
                break;
            case 16:
