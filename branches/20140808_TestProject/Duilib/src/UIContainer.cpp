...
CContainerUI::CContainerUI(const CContainerUI& src) 
: CControlUI(src)
, m_iChildPadding(src.m_iChildPadding)
, m_bAutoDestroy(src.m_bAutoDestroy)
, m_bDelayedDestroy(src.m_bDelayedDestroy)
, m_bMouseChildEnabled(src.m_bMouseChildEnabled)
, m_pVerticalScrollBar(src.m_pVerticalScrollBar)
, m_pHorizontalScrollBar(src.m_pHorizontalScrollBar)
, m_bScrollProcess(src.m_bScrollProcess)
, m_items(src.m_items)
, m_rcInset(src.m_rcInset)
{
	if (src.m_pVerticalScrollBar != NULL)
		this->m_pVerticalScrollBar = new CScrollBarUI(*src.m_pVerticalScrollBar);
	if (src.m_pHorizontalScrollBar != NULL)
		this->m_pHorizontalScrollBar = new CScrollBarUI(*src.m_pHorizontalScrollBar);
}
...
void CContainerUI::RemoveAll()
{
    for( int it = 0; m_bAutoDestroy && it < m_items.GetSize(); it++ ) {
		static_cast<CControlUI*>(m_items[it])->DoDetach();
...
void CContainerUI::DoAttach()
{
	for( int it = 0; it < m_items.GetSize(); it++ ) {
		if( static_cast<CControlUI*>(m_items[it]) != NULL ) {
			static_cast<CControlUI*>(m_items[it])->DoAttach();			
		}
	}
}

void CContainerUI::DoDetach()
{
    for( int it = 0; it < m_items.GetSize(); it++ ) {
        if( static_cast<CControlUI*>(m_items[it]) != NULL ) {
            static_cast<CControlUI*>(m_items[it])->DoDetach();			
...
SIZE CContainerUI::EstimateSize(SIZE szAvailable)
{
	...
	for (int i = 0; i < m_items.GetSize(); ++i) {
		...
		if (m_cxyFixed.cy == 0)
			if (szResult.cy < szItem.cy) 
				szResult.cy = szItem.cy;

		if (m_cxyFixed.cx == 0)
			if (szResult.cx < szItem.cx)
				szResult.cx = szItem.cx;
	}
...
void CVerticalLayoutUI::CalcAdjustItemSize(SIZE *pItemSize, SIZE szAvailable, const CStdPtrArray &aAdjustableItems, int nNotEmpty)
{
	CStdPtrArray aNewAdjustableItems;
	int nAdjustables = aAdjustableItems.GetSize();
	if (nAdjustables == 0) return;

	int x = 0;
	int nRedundancy = 0;
	if (nNotEmpty > 0) {
		int cyExpand = MAX(0, szAvailable.cy / nNotEmpty);
		nRedundancy = szAvailable.cy % nNotEmpty;
		nNotEmpty = 0;
		for (int i = 0; i < aAdjustableItems.GetSize(); ++i) {
			CControlUI* pControl = static_cast<CControlUI*>(m_items[(int)(aAdjustableItems[i])]);
			...
			if (sz.cy != 0) {
				if (sz.cy < cyExpand) {
					pItemSize[(int)(aAdjustableItems[i])].cy = sz.cy;
					szAvailable.cy -= sz.cy;
				}
				else {
					pItemSize[(int)(aAdjustableItems[i])].cy = cyExpand;
					aNewAdjustableItems.Add(aAdjustableItems[i]);
					++x;
				}
				++nNotEmpty;
			}
	...
	if (szAvailable.cy > 0) {
		if (nNotEmpty != 0) {
			if (aNewAdjustableItems.GetSize() != aAdjustableItems.GetSize()) {
				CalcAdjustItemSize(pItemSize, szAvailable, aNewAdjustableItems, x);
			} else {
				if (nRedundancy > 0) {
					pItemSize[(int)(aNewAdjustableItems[aNewAdjustableItems.GetSize() - 1])].cy += nRedundancy;
				}
			}
		} else {
			int nEmpty = nAdjustables - nNotEmpty;
			if (nEmpty != 0) {
				int cyExpand = MAX(0, szAvailable.cy / nEmpty);
				for (int i = 0; i < aAdjustableItems.GetSize(); ++i) {
					pItemSize[(int)(aAdjustableItems[i])].cy = cyExpand;
				}
			}
...
void CVerticalLayoutUI::SetPos(RECT rc)
{
	...
	CStdPtrArray aAdjustableItems;
	SIZE *pItemSize = new SIZE[m_items.GetSize()];
	for (int i = 0; i < m_items.GetSize(); ++i)
		::ZeroMemory(&pItemSize[i], sizeof(SIZE));

	int nNotEmptyAdjustable = 0; // Count of items that is adjustable but not empty (cx not 0).
	for( int it1 = 0; it1 < m_items.GetSize(); it1++ ) {
		...
		else { // Fixed width is 0 indicating the control is adjustable.
			if (sz.cy != 0) ++nNotEmptyAdjustable;
			sz.cy = 0;
			aAdjustableItems.Add((LPVOID)it1);
		}
	...
	int cyNeeded = 0; // Vertical length need to scroll.
	SIZE szRemainingAdjustalbe = { szAvailable.cx, szAvailable.cy - cyFixed };
	if (szRemainingAdjustalbe.cy > 0) {
		CalcAdjustItemSize(pItemSize, szRemainingAdjustalbe, aAdjustableItems, nNotEmptyAdjustable);
	}
	...
	for( int it2 = 0; it2 < m_items.GetSize(); it2++ ) {
		...
		RECT rcCtrl = { iPosX + rcPadding.left, iPosY + rcPadding.top, iPosX + rcPadding.left + sz.cx + rcPadding.right, iPosY + sz.cy + rcPadding.top + rcPadding.bottom };
		pControl->SetPos(rcCtrl);
...
SIZE CVerticalLayoutUI::EstimateSize(SIZE szAvailable)
{
	...
	if (m_cxyFixed.cy == 0) {
		for( int it = 0; it < m_items.GetSize(); it++ ) {
			...
			if (m_cxyFixed.cy == 0) {
				szResult.cy += sz.cy;
				szResult.cy += m_iChildPadding;
			}
		}
		if (szAvailable.cy != CY_INFINITE && szResult.cy > szAvailable.cy)
			szResult.cy = szAvailable.cy;
	}
	else {
		if (m_cxyFixed.cy != 0) szResult.cy = m_cxyFixed.cy;
	}

	szResult.cx = m_cxyFixed.cx;
	return szResult;
}
...
void CHorizontalLayoutUI::CalcAdjustItemSize(SIZE *pItemSize, SIZE szAvailable, const CStdPtrArray &aAdjustableItems, int nNotEmpty)
{
	CStdPtrArray aNewAdjustableItems;
	int nAdjustables = aAdjustableItems.GetSize();
	if (nAdjustables == 0) return;
	...
	int nRedundancy = 0;
	if (nNotEmpty > 0) {
		int cxExpand = MAX(0, szAvailable.cx / nNotEmpty);
		nRedundancy = szAvailable.cx % nNotEmpty;
		nNotEmpty = 0;
		for (int i = 0; i < aAdjustableItems.GetSize(); ++i) {
			CControlUI* pControl = static_cast<CControlUI*>(m_items[(int)(aAdjustableItems[i])]);
			SIZE sz = pControl->EstimateSize(szAvailable);
			...
			if (sz.cx != 0) {
				if (sz.cx < cxExpand) {
					pItemSize[(int)(aAdjustableItems[i])].cx = sz.cx;
					szAvailable.cx -= sz.cx;
				}
				else {
					pItemSize[(int)(aAdjustableItems[i])].cx = cxExpand;
					aNewAdjustableItems.Add(aAdjustableItems[i]);
					++x;
				}
				++nNotEmpty;
	...
	if (szAvailable.cx > 0) {
		if (nNotEmpty != 0) {
			if (aNewAdjustableItems.GetSize() != aAdjustableItems.GetSize()) {
				...
				if (nRedundancy > 0) {
					pItemSize[(int)(aNewAdjustableItems[aNewAdjustableItems.GetSize() - 1])].cx += nRedundancy;
				}
			}
		} else {
			int nEmpty = nAdjustables - nNotEmpty;
			if (nEmpty != 0) {
				int cxExpand = MAX(0, szAvailable.cx / nEmpty);
				for (int i = 0; i < aAdjustableItems.GetSize(); ++i) {
					pItemSize[(int)(aAdjustableItems[i])].cx = cxExpand;
				}
			}
...
void CHorizontalLayoutUI::SetPos(RECT rc)
{
	...
	for( int it1 = 0; it1 < m_items.GetSize(); it1++ ) {
		...
		if (pControl->GetFixedWidth() != 0) { 
			...
			if (sz.cx != 0) ++nNotEmptyAdjustable;
			sz.cx = 0;
			aAdjustableItems.Add((LPVOID)it1);
		}
	...
	int cxNeeded = 0; // Horizontal length need to scroll.
	SIZE szRemainingAdjustalbe = { szAvailable.cx - cxFixed, szAvailable.cy };
	//SIZE szRemainingFixed = { cxFixed, szAvailable.cy };
	if (szRemainingAdjustalbe.cx > 0) {
		CalcAdjustItemSize(pItemSize, szRemainingAdjustalbe, aAdjustableItems, nNotEmptyAdjustable);
	}
	...
	for( int it2 = 0; it2 < m_items.GetSize(); it2++ ) {
		...
		RECT rcCtrl = { iPosX + rcPadding.left, iPosY + rcPadding.top, iPosX + sz.cx + rcPadding.left + rcPadding.right, iPosY + rcPadding.top + sz.cy + rcPadding.bottom };
		pControl->SetPos(rcCtrl);

		iPosX += sz.cx + m_iChildPadding + rcPadding.left + rcPadding.right;
		cxNeeded += sz.cx + rcPadding.left + rcPadding.bottom;
	}
...
SIZE CHorizontalLayoutUI::EstimateSize(SIZE szAvailable)
{
	...
	if (m_cxyFixed.cx == 0) {
		for( int it = 0; it < m_items.GetSize(); it++ ) {
			...
			SIZE sz = pControl->EstimateSize(szAvailable);
			szResult.cx += sz.cx;
			szResult.cx += m_iChildPadding;
		}
		if (szAvailable.cx != CX_INFINITE && szResult.cx > szAvailable.cx)
			szResult.cx = szAvailable.cx;
	}
	else {
		szResult.cx = m_cxyFixed.cx;
	}

	szResult.cy = m_cxyFixed.cy;
	return szResult;
}
...
bool CTabLayoutUI::Remove(CControlUI* pControl)
{
    ...
    if( m_iCurSel == index)
    {
        if( GetCount() > 0 ) /*SelectItem(0);*/ GetItemAt(0)->SetVisible(true);
		else m_iCurSel -= 1;
        NeedParentUpdate();
    }
