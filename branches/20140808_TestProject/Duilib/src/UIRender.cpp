...
TImageInfo* CRenderEngine::LoadImageByStbi(const BYTE* pData, DWORD dwSize, DWORD dwMask)
{
	LPBYTE pImage = NULL;
	int x = 0, y = 0, n = 0;
	...
	ImageFrameInfo.hBitmap = hBitmap;
	ImageFrameInfo.dwTime = 0;
	//data->hBitmap = hBitmap;
	data->FrameInfo.push_back(ImageFrameInfo);
	return data;
}

TImageInfo* CRenderEngine::LoadImageByGdiPlus(const BYTE* pData, DWORD dwSize, DWORD dwMask)
{
	...
	do
	{
		if (NULL == (hGlobal = GlobalAlloc(GMEM_MOVEABLE, dwSize)))
		{
			break;
		}
		if (NULL == (pTempData = GlobalLock(hGlobal)))
		{
			break;
		}
		memcpy(pTempData, pData, dwSize);
		GlobalUnlock(pTempData);
		pTempData = NULL;
		CComPtr<IStream> pIStream;
		if (FAILED(CreateStreamOnHGlobal(hGlobal, TRUE, &pIStream)))
		{
			break;
		}
		hGlobal = NULL;
	...
	TImageInfo* data = new TImageInfo;
	data->nX = pImage->GetWidth();
	data->nY = pImage->GetHeight();
	...
	::ZeroMemory(&bmi, sizeof(BITMAPINFO));
	bmi.bmiHeader.biSize = sizeof(BITMAPINFOHEADER);
	bmi.bmiHeader.biWidth = data->nX;
	bmi.bmiHeader.biHeight = -data->nY;
	bmi.bmiHeader.biPlanes = 1;
	bmi.bmiHeader.biBitCount = 32;
	bmi.bmiHeader.biCompression = BI_RGB;
	bmi.bmiHeader.biSizeImage = data->nX * data->nY * 4;
	...
	if (nFrameCount > 1)
	{
		...
		pItem = (PropertyItem*)(new char[nSize]);

		pImage->GetPropertyItem(PropertyTagFrameDelay, nSize, pItem);
		ASSERT(sizeof(long) * nFrameCount == pItem->length);
	}
	
	for (UINT nFrameIndex = 0; nFrameIndex < nFrameCount; nFrameIndex++)
	{
		GUID    Guid = FrameDimensionTime;
		if (Ok != (status = pImage->SelectActiveFrame(&Guid, nFrameIndex)))
		{
			ASSERT(FALSE);
			continue;
		}
		...
		ASSERT(!IsBadWritePtr(pDest, bmi.bmiHeader.biSizeImage));
		Color PixelColor = Color::Black;

		for (int nX = 0; nX < data->nX; nX++)
		{
			for (int nY = 0; nY < data->nY; nY++)
			{
				if (Ok != (status = pImage->GetPixel(nX, nY, &PixelColor)))
				{
					ASSERT(FALSE);
					continue;
				}
				LPBYTE pDest0 = NULL, pDest1 = NULL, pDest2 = NULL, pDest3 = NULL;
				pDest0 = pDest + (nY * data->nX + nX) * sizeof(DWORD);	//blue
				pDest1 = pDest0 + 1;		//green
				pDest2 = pDest0 + 2;		//red
				pDest3 = pDest0 + 3;		//AlphaChannel
				*pDest3 = PixelColor.GetAlpha();	
				if( *pDest3 < 255 )
				{
					*pDest0 = (BYTE)((DWORD)PixelColor.GetB() * PixelColor.GetAlpha() / 255);
					*pDest1 = (BYTE)((DWORD)PixelColor.GetG() * PixelColor.GetAlpha() / 255);
					*pDest2 = (BYTE)((DWORD)PixelColor.GetR() * PixelColor.GetAlpha() / 255); 
					bAlphaChannel = true;
				}
				else
				{
					*pDest0 = PixelColor.GetB();
					*pDest1 = PixelColor.GetG();
					*pDest2 = PixelColor.GetR(); 
				}

				if( *(DWORD*)(pDest0) == dwMask ) {
					*pDest0 = (BYTE)0;
					*pDest1 = (BYTE)0;
					*pDest2 = (BYTE)0; 
					*pDest3 = (BYTE)0;
		...
		IMAGE_FRAME_INFO ImageFrameInfo;
		ImageFrameInfo.hBitmap = hBitmap;
		if (NULL != pItem)	//只有多帧的时候，才有效
		{
			long lPause = ((long*)pItem->value)[nFrameIndex];
			lPause *= 10;//例子代码*10为毫秒
			ImageFrameInfo.dwTime = lPause;
		}
		else
		{
			ImageFrameInfo.dwTime = 0;
...
TImageInfo* CRenderEngine::LoadImage(STRINGorID bitmap, LPCTSTR type, DWORD mask)
{
	...
	CString strFileName(bitmap.m_lpstr);
	strFileName.MakeLower();
	if (strFileName.Right(4) == _T(".gif"))
	{
		pImageInfo = LoadImageByGdiPlus(pData, dwSize, mask);
	}
	else
	{
		pImageInfo = LoadImageByStbi(pData, dwSize, mask);
		if (NULL == pImageInfo)
		{
			pImageInfo = LoadImageByGdiPlus(pData, dwSize, mask);
		}
...
{
	for (IMAGE_FRAME_INFO_ARRAY::const_iterator itorImageFrame = bitmap->FrameInfo.begin();
		itorImageFrame != bitmap->FrameInfo.end();
		++itorImageFrame)
	{
		const IMAGE_FRAME_INFO &ImageFrameInfo = *itorImageFrame;
		if (NULL != ImageFrameInfo.hBitmap)
		{
			::DeleteObject(ImageFrameInfo.hBitmap);
		}
	}
	/*if (bitmap->hBitmap) {
...
bool DrawImage(HDC hDC, CPaintManagerUI* pManager, const RECT& rc, const RECT& rcPaint, const CStdString& sImageName, \
			   ...
			   bool bHole, bool bGray, bool bTiledX, bool bTiledY, DWORD dwFrameIndex, const TImageInfo **ppImageInfo)
{
	const TImageInfo* data = NULL;
	if (bGray) {
		CStdString sGrayImageName = sImageName + _T(".gray");
		// Get gray image.
		data = pManager->GetImage((LPCTSTR)sGrayImageName);
		// No gray image, create gray image.
		if (!data) {
			...
			//HGDIOBJ hOldBitmap1 = ::SelectObject(hMemDC1, data->hBitmap);
			HGDIOBJ hOldBitmap1 = ::SelectObject(hMemDC1, data->FrameInfo[dwFrameIndex].hBitmap);

			HDC hMemDC = ::CreateCompatibleDC(hMemDC1);
			HBITMAP hBmp = ::CreateCompatibleBitmap(hMemDC1, data->nX, data->nY);
			HGDIOBJ hOldBitmap = ::SelectObject(hMemDC, hBmp);
			::BitBlt(hMemDC, 0, 0, data->nX, data->nY, hMemDC1, 0, 0, SRCCOPY);
			...
			BITMAP bmp1;
			//::GetObject(data->hBitmap, sizeof(BITMAP), &bmp1);
			::GetObject(data->FrameInfo[dwFrameIndex].hBitmap, sizeof(BITMAP), &bmp1);
			BITMAP bmp;
			::GetObject(hBmp, sizeof(BITMAP), &bmp);
			LPBYTE pDest = (LPBYTE)bmp.bmBits;
			if (pDest == NULL) return false;

			for (int i = 0; i < data->nX * data->nY; ++i)
			{
				BYTE byRed = pDest[i * 4];
				BYTE byGreen = pDest[i * 4  + 1];
				BYTE byBlue = pDest[i * 4 + 2];
				BYTE byGray = byRed * 0.299 + byGreen * 0.587 + byBlue * 0.114;
				pDest[i * 4] = pDest[i * 4 + 1] = pDest[i * 4 + 2] = byGray;
			}

			data = pManager->AddImage(sGrayImageName, hBmp, data->nX, data->nY, data->alphaChannel);
		}
	...
	CRenderEngine::DrawImage(hDC, data->FrameInfo[dwFrameIndex].hBitmap, rcItem, rcPaint, rcBmpPart, rcCorner, data->alphaChannel, bFade, bHole, bTiledX, bTiledY);
	if (NULL != ppImageInfo)
	{
		*ppImageInfo = data;
	}
...
bool CRenderEngine::DrawImageString(HDC hDC, CPaintManagerUI* pManager, const RECT& rc, const RECT& rcPaint, 
									LPCTSTR pStrImage, LPCTSTR pStrModify, DWORD dwFrameIndex, const TImageInfo **ppImageInfo)
{
	if ((pManager == NULL) || (hDC == NULL)) return false;

	// 1、aaa.jpg
	// 2、file='aaa.jpg' res='' restype='0' dest='0,0,0,0' source='0,0,0,0' corner='0,0,0,0' 
	// mask='#FF0000' fade='255' hole='false' gray='false' xtiled='false' ytiled='false'
			...
			if( !sValue.IsEmpty() ) {
				if( sItem == _T("file") || sItem == _T("res") ) {
					if( image_count > 0 )
						...
						rcItem, rcBmpPart, rcCorner, dwMask, bFade, bHole, bGray, bTiledX, bTiledY, dwFrameIndex, ppImageInfo);
                            ...
                            if( pt.x + iWidth > rc.right && pt.x > rc.left && (uStyle & DT_SINGLELINE) == 0 ) {
                                ...
                                if( bDraw && bLineDraw ) {
                                   ...
                                   /* DrawImage(hDC, pImageInfo->hBitmap, rcImage, rcImage, rcBmpPart, rcCorner, \
									...
									//这里没有进行多帧图片绘制的逻辑，以后再添加
									DrawImage(hDC, pImageInfo->FrameInfo[0].hBitmap, rcImage, rcImage, rcBmpPart, rcCorner, \
										pImageInfo->alphaChannel, 255);
                                }
