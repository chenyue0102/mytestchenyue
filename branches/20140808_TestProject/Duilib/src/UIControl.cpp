...
namespace DuiLib {
CFrameImageHelp g_FrameImageHelp;
...
m_dwBackColor3(0),
m_dwBorderColor(0),
m_bVerticalGradient(true),
m_dwFocusBorderColor(0),
m_bColorHSL(false),
m_nBorderSize(1),
m_pImagePlayStateInfoArray(new ImagePlayStateInfoArray)
{
    m_cXY.cx = m_cXY.cy = 0;
...
CControlUI::~CControlUI()
{
	...
	if (NULL != m_pImagePlayStateInfoArray)
	{
		delete m_pImagePlayStateInfoArray;
		m_pImagePlayStateInfoArray = NULL;
	}
}
...
void CControlUI::SetVerticalGradient(bool bVertial)
{
	if (m_bVerticalGradient == bVertial) return;

	m_bVerticalGradient = bVertial;
	Invalidate();
}

bool CControlUI::IsVerticalGradient() const
{
	return m_bVerticalGradient;
}
...
void CControlUI::SetBkImage(LPCTSTR pStrImage)
{
	...
	StopImagePlayTimerAndCleanFrameInfo(FRAME_TYPE_Control_BkImage);//如果定时器存在,停止这个图片的定时器
...
bool CControlUI::DrawImage(HDC hDC, LPCTSTR pStrImage, LPCTSTR pStrModify, FRAME_TYPE FrameType/* = FRAME_TYPE_NONE*/)
{
	const TImageInfo *pImageInfo = NULL;
	DWORD dwCurFrame = 0;
	ImagePlayStateInfo *pFrameInfo = NULL;;
	if (FRAME_TYPE_NONE != FrameType)
	{
		ImagePlayStateInfo &FrameInfo = GetFrameInfo(FrameType);
		dwCurFrame = FrameInfo.dwCurFrame;
		pFrameInfo = &FrameInfo;
	}
    bool bResult = CRenderEngine::DrawImageString(hDC, m_pManager, m_rcItem, m_rcPaint, pStrImage, pStrModify, dwCurFrame, &pImageInfo);
	if (bResult && NULL != pImageInfo && pImageInfo->FrameInfo.size() > 1)
	{
		//多帧，则需要启动定时器
		if (FRAME_TYPE_NONE != FrameType)
		{
			if (pFrameInfo->bPlayAnimate)
			{
				if (!g_FrameImageHelp.IsTimerExist(this, FrameType))
				{
					g_FrameImageHelp.StartTimer(this, FrameType, pImageInfo);
				}
			}
		}
		else
...
void CControlUI::DoAttach()
{

}

void CControlUI::DoDetach()
{
	g_FrameImageHelp.StopAllTimer(this);
	/*m_pManager = NULL;
	m_pParent = NULL;*/
}
...
void CControlUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
	...
	else if (_tcscmp(pstrName, _T("verticalgradient")) == 0) {
		SetVerticalGradient(_tcscmp(pstrValue, _T("true")) == 0);
	}
    ...
    if( m_dwBackColor != 0 ) {
        if( m_dwBackColor2 != 0 ) {
            if( m_dwBackColor3 != 0 ) {
                ...
                CRenderEngine::DrawGradient(hDC, rc, GetAdjustColor(m_dwBackColor), GetAdjustColor(m_dwBackColor2), m_bVerticalGradient, 8);
				if (m_bVerticalGradient) {
					rc.top = rc.bottom;
                ...
                CRenderEngine::DrawGradient(hDC, rc, GetAdjustColor(m_dwBackColor2), GetAdjustColor(m_dwBackColor3), m_bVerticalGradient, 8);
            }
            else 
                CRenderEngine::DrawGradient(hDC, m_rcItem, GetAdjustColor(m_dwBackColor), GetAdjustColor(m_dwBackColor2), m_bVerticalGradient, 16);
        }
...
void CControlUI::PaintBkImage(HDC hDC)
{
    ...
    if( !DrawImage(hDC, (LPCTSTR)m_sBkImage, NULL, FRAME_TYPE_Control_BkImage) ) m_sBkImage.Empty();
}
...
ImagePlayStateInfo&		CControlUI::GetFrameInfo(FRAME_TYPE FrameType)
{
	ASSERT(FRAME_TYPE_NONE != FrameType);
	ImagePlayStateInfoArray::iterator itorImagePlayState = m_pImagePlayStateInfoArray->find(FrameType);
	if (itorImagePlayState == m_pImagePlayStateInfoArray->end())//没有找到就创建一个添加进去
	{
		ImagePlayStateInfo NewPlayStateInfo;
		NewPlayStateInfo.bPlayAnimate = TRUE;	//默认是播放多帧图片的
		NewPlayStateInfo.dwCurFrame = 0;
		m_pImagePlayStateInfoArray->insert(std::make_pair(FrameType, NewPlayStateInfo));
		itorImagePlayState = m_pImagePlayStateInfoArray->find(FrameType);
	}
	return itorImagePlayState->second;
}

bool		CControlUI::StopImagePlayTimer(FRAME_TYPE FrameType)
{
	g_FrameImageHelp.StopTimer(this, FrameType);//如果定时器存在,停止这个图片的定时器
	return true;
}

bool		CControlUI::SetImagePlayState(FRAME_TYPE FrameType, bool bPlayAnimate)
{
	ImagePlayStateInfo &ImageFrameInfo = GetFrameInfo(FrameType);
	ImageFrameInfo.bPlayAnimate = static_cast<BOOL>(bPlayAnimate);
	if (!bPlayAnimate)	//停止播放，则停止定时器，启动播放的时候，绘制时会启动
	{
		StopImagePlayTimer(FrameType);
		ImageFrameInfo.dwCurFrame = 0;		//停止播放，则显示第一帧
	}
	return true;
}

bool		CControlUI::StopImagePlayTimerAndCleanFrameInfo(FRAME_TYPE FrameType)
{
	StopImagePlayTimer(FrameType);
	//将图片信息回复默认
	ImagePlayStateInfo &ImageFrameInfo = GetFrameInfo(FrameType);
	ImageFrameInfo.bPlayAnimate = TRUE;
