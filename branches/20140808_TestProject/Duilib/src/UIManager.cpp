...
HDC CPaintManagerUI::GetPaintDCAgain()
{
    if (NULL != m_hDcPaint)
    {
...
{
	if (0 == m_szInitWindowSize.cx || 0 == m_szInitWindowSize.cy)
	{
                    ...
                    if( m_pRoot->IsUpdateNeeded()) {
						...
						else
						{
							...
							while( pControl = m_pRoot->FindControl(__FindControlFromUpdate, NULL, UIFIND_VISIBLE | UIFIND_ME_FIRST) ) {
								...
								while (true) {
									if (!::IsRectEmpty(&rcControl)) {
										SIZE szClient = { rcClient.bottom - rcClient.top, rcClient.right - rcClient.left };
										SIZE szEstimate = pControl->EstimateSize(szClient);
										if (rcControl.bottom - rcControl.top == szEstimate.cy
											&& rcControl.right - rcControl.left == szEstimate.cx)
										{
											// Need to update the parent's rect.
    ...
    case WM_LBUTTONDBLCLK:
        {
            ...
            if( pControl == NULL ) break;
            if( pControl->GetManager() != this ) break;
			// 双击打开聊天窗口后，焦点给了richedit，造成无法调用对应的WM_LBUTTONUP里的ReleaseCapture
            //SetCapture();
            TEventUI event = { 0 };
...
bool CPaintManagerUI::AddMessageFilter(IMessageFilterUI* pFilter)
{
	...
	{
		bResult = m_aMessageFilters.Add(pFilter);
	}
	return bResult;
}
...
const TImageInfo* CPaintManagerUI::AddImage(LPCTSTR bitmap, LPCTSTR type, DWORD mask)
{
    ...
    if( !m_mImageHash.Insert(bitmap, data) ) {
		for (IMAGE_FRAME_INFO_ARRAY::const_iterator itorImageFrame = data->FrameInfo.begin();
			itorImageFrame != data->FrameInfo.end();
			++itorImageFrame)
		{
    ...
    for( int i = 0; i< m_mImageHash.GetSize(); i++ ) {
        if(LPCTSTR bitmap = m_mImageHash.GetAt(i)) {
            ...
            if( data != NULL ) {
				...
				data->FrameInfo.clear();
				data->FrameInfo = pNewData->FrameInfo;
                data->nX = pNewData->nX;
                data->nY = pNewData->nY;
...
bool CPaintManagerUI::IsInited()
{
	return m_bInited;
}

void CPaintManagerUI::SetInited(bool bInited/* = true*/)
{
	m_bInited = bInited;
}

bool CPaintManagerUI::IsEnableResizeWindow()
{
	return m_bEnableResizeWindow;
}

void CPaintManagerUI::SetEnableResizeWindow(bool bEnableResize)
{
	m_bEnableResizeWindow = bEnableResize;
}

bool CPaintManagerUI::FitWindowSize()
{
	RECT rcClient = m_pRoot->GetPos();
	SIZE szClient = { rcClient.right - rcClient.left, rcClient.bottom - rcClient.top }, szClientMax = GetMaxInfo();
	...
	SIZE szEstimate = m_pRoot->EstimateSize(szClientMax);
	if (szClient.cx == szEstimate.cx && szClient.cy == szEstimate.cy)
	{
		//当前大小合适
		m_pRoot->SetPos( rcClient);
	}
	else
	{
		//需要修改窗口大小
		RECT rcWindow = {0};
		::GetWindowRect(m_hWndPaint, &rcWindow);
		rcWindow.right += szEstimate.cx - szClient.cx;
		rcWindow.bottom += szEstimate.cy - szClient.cy;
		MoveWindow(m_hWndPaint, rcWindow.left, rcWindow.top, rcWindow.right - rcWindow.left, rcWindow.bottom - rcWindow.top, FALSE);
	}
	return true;
}
