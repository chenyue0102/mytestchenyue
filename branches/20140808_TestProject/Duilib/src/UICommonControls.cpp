...
SIZE CLabelUI::EstimateSize(SIZE szAvailable)
{
	...
	{
		szResult = CControlUI::EstimateSize(szAvailable);
	}
...
void CButtonUI::SetNormalImage(LPCTSTR pStrImage)
{
	...
	StopImagePlayTimerAndCleanFrameInfo(FRAME_TYPE_Button_NormalImage);//如果定时器存在,停止这个图片的定时器
    Invalidate();
}
...
void CButtonUI::SetHotImage(LPCTSTR pStrImage)
{
	...
	StopImagePlayTimerAndCleanFrameInfo(FRAME_TYPE_Button_HotImage);//如果定时器存在,停止这个图片的定时器
    Invalidate();
}
...
void CButtonUI::SetPushedImage(LPCTSTR pStrImage)
{
	...
	StopImagePlayTimerAndCleanFrameInfo(FRAME_TYPE_Button_PushedImage);//如果定时器存在,停止这个图片的定时器
    Invalidate();
}
...
void CButtonUI::SetFocusedImage(LPCTSTR pStrImage)
{
	...
	StopImagePlayTimerAndCleanFrameInfo(FRAME_TYPE_Button_FocusedImage);//如果定时器存在,停止这个图片的定时器
    Invalidate();
}
...
void CButtonUI::SetDisabledImage(LPCTSTR pStrImage)
{
	...
	StopImagePlayTimerAndCleanFrameInfo(FRAME_TYPE_Button_DisabledImage);//如果定时器存在,停止这个图片的定时器
    Invalidate();
}
...
void CButtonUI::PaintStatusImage(HDC hDC)
{
    ...
    if( (m_uButtonState & UISTATE_DISABLED) != 0 ) {
        if( !m_sDisabledImage.IsEmpty() ) {
            if( !DrawImage(hDC, (LPCTSTR)m_sDisabledImage, NULL, FRAME_TYPE_Button_DisabledImage) ) m_sDisabledImage.Empty();
            else return;
        }
    }
    else if( (m_uButtonState & UISTATE_PUSHED) != 0 ) {
        if( !m_sPushedImage.IsEmpty() ) {
            if( !DrawImage(hDC, (LPCTSTR)m_sPushedImage, NULL, FRAME_TYPE_Button_PushedImage) ) m_sPushedImage.Empty();
            else return;
        }
    }
    else if( (m_uButtonState & UISTATE_HOT) != 0 ) {
        if( !m_sHotImage.IsEmpty() ) {
            if( !DrawImage(hDC, (LPCTSTR)m_sHotImage, NULL, FRAME_TYPE_Button_HotImage) ) m_sHotImage.Empty();
            else return;
        }
    }
    else if( (m_uButtonState & UISTATE_FOCUSED) != 0 ) {
        if( !m_sFocusedImage.IsEmpty() ) {
            if( !DrawImage(hDC, (LPCTSTR)m_sFocusedImage, NULL, FRAME_TYPE_Button_FocusedImage) ) m_sFocusedImage.Empty();
            else return;
        }
    }

    if( !m_sNormalImage.IsEmpty() ) {
        if( !DrawImage(hDC, (LPCTSTR)m_sNormalImage, NULL, FRAME_TYPE_Button_NormalImage) ) m_sNormalImage.Empty();
        else return;
    }
}
...
void COptionUI::SetSelectedImage(LPCTSTR pStrImage)
{
	...
	StopImagePlayTimerAndCleanFrameInfo(FRAME_TYPE_Option_SelectedImage);//如果定时器存在,停止这个图片的定时器
    Invalidate();
}
...
void COptionUI::SetForeImage(LPCTSTR pStrImage)
{
	...
	StopImagePlayTimerAndCleanFrameInfo(FRAME_TYPE_Option_ForeImage);//如果定时器存在,停止这个图片的定时器
    Invalidate();
}
...
void COptionUI::SetSelectedForeImage(LPCTSTR pStrImage)
{
	...
	StopImagePlayTimerAndCleanFrameInfo(FRAME_TYPE_Option_SelectedForeImage);//如果定时器存在,停止这个图片的定时器
    if( m_bSelected )
    {
        Invalidate();
    }
...
void COptionUI::PaintStatusImage(HDC hDC)
{
    ...
    if( (m_uButtonState & UISTATE_SELECTED) != 0 ) {
        if( !m_sSelectedImage.IsEmpty() ) {
            if( !DrawImage(hDC, (LPCTSTR)m_sSelectedImage, NULL, FRAME_TYPE_Option_SelectedImage) ) m_sSelectedImage.Empty();
...
Label_ForeImage:
    if( m_bSelected && !m_sSelectedForeImage.IsEmpty() )
    {
        if( !DrawImage(hDC, (LPCTSTR)m_sSelectedForeImage, NULL, FRAME_TYPE_Option_SelectedForeImage) )
            m_sSelectedForeImage.Empty();
    }
	else if( !m_sForeImage.IsEmpty() ) {
		if( !DrawImage(hDC, (LPCTSTR)m_sForeImage, NULL, FRAME_TYPE_Option_ForeImage) ) m_sForeImage.Empty();
	}
}
...
void CProgressUI::SetForeImage(LPCTSTR pStrImage)
{
    ...
    m_sForeImage = pStrImage;
	StopImagePlayTimerAndCleanFrameInfo(FRAME_TYPE_Progress_ForeImage);//如果定时器存在,停止这个图片的定时器
    Invalidate();
}
...
void CProgressUI::PaintStatusImage(HDC hDC)
{
    ...
    if( !m_sForeImage.IsEmpty() ) {
        ...
        if( !DrawImage(hDC, (LPCTSTR)m_sForeImage, (LPCTSTR)m_sForeImageModify, FRAME_TYPE_Progress_ForeImage) ) m_sForeImage.Empty();
        else return;
    }
}
...
void CSliderUI::SetThumbImage(LPCTSTR pStrImage)
{
	...
	StopImagePlayTimerAndCleanFrameInfo(FRAME_TYPE_Slider_ThumbImage);//如果定时器存在,停止这个图片的定时器
    Invalidate();
}
...
void CSliderUI::SetThumbHotImage(LPCTSTR pStrImage)
{
	...
	StopImagePlayTimerAndCleanFrameInfo(FRAME_TYPE_Slider_ThumbHotImage);//如果定时器存在,停止这个图片的定时器
    Invalidate();
}
...
void CSliderUI::SetThumbPushedImage(LPCTSTR pStrImage)
{
	...
	StopImagePlayTimerAndCleanFrameInfo(FRAME_TYPE_Slider_ThumbPushedImage);//如果定时器存在,停止这个图片的定时器
    Invalidate();
}
...
void CSliderUI::PaintStatusImage(HDC hDC)
{
    ...
    if( (m_uButtonState & UISTATE_CAPTURED) != 0 ) {
        if( !m_sThumbPushedImage.IsEmpty() ) {
            ...
            if( !DrawImage(hDC, (LPCTSTR)m_sThumbPushedImage, (LPCTSTR)m_sImageModify, FRAME_TYPE_Slider_ThumbPushedImage) ) m_sThumbPushedImage.Empty();
            else return;
        }
    }
    else if( (m_uButtonState & UISTATE_HOT) != 0 ) {
        if( !m_sThumbHotImage.IsEmpty() ) {
            m_sImageModify.Empty();
            m_sImageModify.SmallFormat(_T("dest='%d,%d,%d,%d'"), rcThumb.left, rcThumb.top, rcThumb.right, rcThumb.bottom);
            if( !DrawImage(hDC, (LPCTSTR)m_sThumbHotImage, (LPCTSTR)m_sImageModify, FRAME_TYPE_Slider_ThumbHotImage) ) m_sThumbHotImage.Empty();
            else return;
        }
    }
    ...
    if( !m_sThumbImage.IsEmpty() ) {
        m_sImageModify.Empty();
        m_sImageModify.SmallFormat(_T("dest='%d,%d,%d,%d'"), rcThumb.left, rcThumb.top, rcThumb.right, rcThumb.bottom);
        if( !DrawImage(hDC, (LPCTSTR)m_sThumbImage, (LPCTSTR)m_sImageModify, FRAME_TYPE_Slider_ThumbImage) ) m_sThumbImage.Empty();
        else return;
    }
}
...
void CEditWnd::Init(CEditUI* pOwner)
{
	...
	else
	{
		...
		SendMessage(WM_LBUTTONDOWN, 0, MAKELPARAM(pt.x, pt.y));
...
CEditUI::CEditUI() : m_pWindow(NULL), m_uMaxChar(255), m_bReadOnly(false), m_bNumberOnly(false),
m_bPasswordMode(false), m_cPasswordChar(_T('*')), m_uButtonState(0), m_dwEditbkColor(0xFFFFFFFF),m_bMultiLine(false),m_bHideCaret(false)
,m_dwEmptyTipTextColor(0xFF7E7E7E)
{
    ...
    if( event.Type == UIEVENT_BUTTONDOWN || event.Type == UIEVENT_DBLCLICK || event.Type == UIEVENT_RBUTTONDOWN) 
    {
        if( IsEnabled() ) {
            ...
            if( IsFocused() && m_pWindow == NULL )
            {
				...
				if( PtInRect(&m_rcItem, event.ptMouse) && event.Type == UIEVENT_DBLCLICK)		//双击的时候，才自动选中，
				{
            ...
            else if( m_pWindow != NULL )
            {
				...
				{
					Edit_SetSel(*m_pWindow, 0, nSize);				//双击的时候，才自动选中，
				}
#else
...
void CEditUI::SetSel(int nStartChar, int nEndChar, BOOL bNoScroll/* = FALSE*/)
{
	if (NULL == m_pWindow)
	{
		m_pWindow = new CEditWnd;
	...
	::SendMessage(hEdit, EM_SETSEL, nStartChar, nEndChar);
	if (!bNoScroll)
	{
...
{
    m_sNormalImage = pStrImage;
	StopImagePlayTimerAndCleanFrameInfo(FRAME_TYPE_Edit_NormalImage);//如果定时器存在,停止这个图片的定时器
    Invalidate();
}
...
{
    m_sHotImage = pStrImage;
	StopImagePlayTimerAndCleanFrameInfo(FRAME_TYPE_Edit_HotImage);//如果定时器存在,停止这个图片的定时器
    Invalidate();
}
...
{
    m_sFocusedImage = pStrImage;
	StopImagePlayTimerAndCleanFrameInfo(FRAME_TYPE_Edit_FocusedImage);//如果定时器存在,停止这个图片的定时器
    Invalidate();
}
...
{
    m_sDisabledImage = pStrImage;
	StopImagePlayTimerAndCleanFrameInfo(FRAME_TYPE_Edit_DisabledImage);//如果定时器存在,停止这个图片的定时器
    Invalidate();
}
...
void CEditUI::SetMultiLine(bool bMultiLine)
{
	if (bMultiLine)
	{
		m_uTextStyle = DT_WORDBREAK | DT_EDITCONTROL| DT_EDITCONTROL|DT_LEFT;
	}
	else
	{
		m_uTextStyle = DT_SINGLELINE | DT_CENTER;
...
void CEditUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
	...
	else if (_tcscmp(pstrName, _T("hidecaret")) == 0)
	{
		SetHideCaret(_tcscmp(pstrValue, _T("true")) == 0);
	}
	else if (_tcscmp(pstrName, _T("emptytiptext")) == 0)
	{
		SetEmptyTipText(pstrValue);
	}
	else if (_tcscmp(pstrName, _T("emptytiptextcolor")) == 0)
	{
		LPTSTR pstr = NULL;
		DWORD clrColor = _tcstoul(pstrValue, &pstr, 16);
		SetEmptyTipTextColor(clrColor);
	}
    else CLabelUI::SetAttribute(pstrName, pstrValue);
}
...
void CEditUI::PaintStatusImage(HDC hDC)
{
    ...
    if( (m_uButtonState & UISTATE_DISABLED) != 0 ) {
        if( !m_sDisabledImage.IsEmpty() ) {
            if( !DrawImage(hDC, (LPCTSTR)m_sDisabledImage, NULL, FRAME_TYPE_Edit_DisabledImage) ) m_sDisabledImage.Empty();
            else return;
        }
    }
    else if( (m_uButtonState & UISTATE_FOCUSED) != 0 ) {
        if( !m_sFocusedImage.IsEmpty() ) {
            if( !DrawImage(hDC, (LPCTSTR)m_sFocusedImage, NULL, FRAME_TYPE_Edit_FocusedImage) ) m_sFocusedImage.Empty();
            else return;
        }
    }
    else if( (m_uButtonState & UISTATE_HOT) != 0 ) {
        if( !m_sHotImage.IsEmpty() ) {
            if( !DrawImage(hDC, (LPCTSTR)m_sHotImage, NULL, FRAME_TYPE_Edit_HotImage) ) m_sHotImage.Empty();
            else return;
        }
    }

    if( !m_sNormalImage.IsEmpty() ) {
        if( !DrawImage(hDC, (LPCTSTR)m_sNormalImage, NULL, FRAME_TYPE_Edit_NormalImage) ) m_sNormalImage.Empty();
        else return;
    }
}
...
void CEditUI::PaintText(HDC hDC)
{
    ...
    if( m_sText.IsEmpty() ) 
	{
		if (!m_sEmptyTipText.IsEmpty())
		{
			...
			rc.top += m_rcTextPadding.top;
			rc.bottom -= m_rcTextPadding.bottom;
			CRenderEngine::DrawText(hDC, m_pManager, rc, m_sEmptyTipText, m_dwEmptyTipTextColor, \
				m_iFont, m_uTextStyle);
		}
		return;
	}
...
bool	CEditUI::IsHideCaret()
{
	return m_bHideCaret;
}

void CEditUI::SetHideCaret(bool bHideCaret)
{
	if (m_bHideCaret == bHideCaret)
	{
		return;
	}
	m_bHideCaret = bHideCaret;
	if (NULL != m_pWindow && NULL != m_pWindow->GetHWND())
	{
		if (m_bHideCaret)
		{
...
void CEditUI::SetEmptyTipText(LPCTSTR lpText)
{
	m_sEmptyTipText = lpText;
	Invalidate();
}

CStdString CEditUI::GetEmptyTipText()
{
	return m_sEmptyTipText;
}

void CEditUI::SetEmptyTipTextColor(DWORD dwEmptyTipTextColor)
{
	if (m_dwEmptyTipTextColor != dwEmptyTipTextColor)
	{
		m_dwEmptyTipTextColor = dwEmptyTipTextColor;
		Invalidate();
	}
}

DWORD	CEditUI::GetEmptyTipTextColor()
{
	return m_dwEmptyTipTextColor;
}
...
m_pOwner(NULL), m_nLastScrollPos(0), m_nLastScrollOffset(0), m_nScrollRepeatDelay(0), m_uButton0State(0), m_uButton1State(0), \
m_uButton2State(0), m_uButton3State(0), m_uThumbState(0), m_bShowButton0(false), m_bShowButton1(true), m_bShowButton2(true), m_bShowButton3(false)
{
    m_cxyFixed.cx = DEFAULT_SCROLLBAR_SIZE;
    ptLastMouse.x = ptLastMouse.y = 0;
    ::ZeroMemory(&m_rcThumb, sizeof(m_rcThumb));
	::ZeroMemory(&m_rcButton0, sizeof(m_rcButton0));
    ::ZeroMemory(&m_rcButton1, sizeof(m_rcButton1));
    ::ZeroMemory(&m_rcButton2, sizeof(m_rcButton2));
	::ZeroMemory(&m_rcButton3, sizeof(m_rcButton3));
}
...
bool CScrollBarUI::GetShowButton0()
{
	return m_bShowButton0;
}

void CScrollBarUI::SetShowButton0(bool bShow)
{
	m_bShowButton0 = bShow;
	SetPos(m_rcItem);
}

LPCTSTR CScrollBarUI::GetButton0NormalImage()
{
	return m_sButton0NormalImage;
}

void CScrollBarUI::SetButton0NormalImage(LPCTSTR pStrImage)
{
	m_sButton0NormalImage = pStrImage;
	StopImagePlayTimerAndCleanFrameInfo(FRAME_TYPE_ScrollBar_Button0NormalImage);//如果定时器存在,停止这个图片的定时器
	Invalidate();
}

LPCTSTR CScrollBarUI::GetButton0HotImage()
{
	return m_sButton0HotImage;
}

void CScrollBarUI::SetButton0HotImage(LPCTSTR pStrImage)
{
	m_sButton0HotImage = pStrImage;
	StopImagePlayTimerAndCleanFrameInfo(FRAME_TYPE_ScrollBar_Button0HotImage);//如果定时器存在,停止这个图片的定时器
	Invalidate();
}

LPCTSTR CScrollBarUI::GetButton0PushedImage()
{
	return m_sButton0PushedImage;
}

void CScrollBarUI::SetButton0PushedImage(LPCTSTR pStrImage)
{
	m_sButton0PushedImage = pStrImage;
	StopImagePlayTimerAndCleanFrameInfo(FRAME_TYPE_ScrollBar_Button0PushedImage);//如果定时器存在,停止这个图片的定时器
	Invalidate();
}

LPCTSTR CScrollBarUI::GetButton0DisabledImage()
{
	return m_sButton0DisabledImage;
}

void CScrollBarUI::SetButton0DisabledImage(LPCTSTR pStrImage)
{
	m_sButton0DisabledImage = pStrImage;
	StopImagePlayTimerAndCleanFrameInfo(FRAME_TYPE_ScrollBar_Button0DisabledImage);//如果定时器存在,停止这个图片的定时器
	Invalidate();
}
...
void CScrollBarUI::SetButton1NormalImage(LPCTSTR pStrImage)
{
	...
	StopImagePlayTimerAndCleanFrameInfo(FRAME_TYPE_ScrollBar_Button1NormalImage);//如果定时器存在,停止这个图片的定时器
    Invalidate();
}
...
void CScrollBarUI::SetButton1HotImage(LPCTSTR pStrImage)
{
	...
	StopImagePlayTimerAndCleanFrameInfo(FRAME_TYPE_ScrollBar_Button1HotImage);//如果定时器存在,停止这个图片的定时器
    Invalidate();
}
...
void CScrollBarUI::SetButton1PushedImage(LPCTSTR pStrImage)
{
	...
	StopImagePlayTimerAndCleanFrameInfo(FRAME_TYPE_ScrollBar_Button1PushedImage);//如果定时器存在,停止这个图片的定时器
    Invalidate();
}
...
void CScrollBarUI::SetButton1DisabledImage(LPCTSTR pStrImage)
{
	...
	StopImagePlayTimerAndCleanFrameInfo(FRAME_TYPE_ScrollBar_Button1DisabledImage);//如果定时器存在,停止这个图片的定时器
    Invalidate();
}
...
void CScrollBarUI::SetButton2NormalImage(LPCTSTR pStrImage)
{
	...
	StopImagePlayTimerAndCleanFrameInfo(FRAME_TYPE_ScrollBar_Button2NormalImage);//如果定时器存在,停止这个图片的定时器
    Invalidate();
}
...
void CScrollBarUI::SetButton2HotImage(LPCTSTR pStrImage)
{
	...
	StopImagePlayTimerAndCleanFrameInfo(FRAME_TYPE_ScrollBar_Button2HotImage);//如果定时器存在,停止这个图片的定时器
    Invalidate();
}
...
void CScrollBarUI::SetButton2PushedImage(LPCTSTR pStrImage)
{
	...
	StopImagePlayTimerAndCleanFrameInfo(FRAME_TYPE_ScrollBar_Button2PushedImage);//如果定时器存在,停止这个图片的定时器
    Invalidate();
}
...
void CScrollBarUI::SetButton2DisabledImage(LPCTSTR pStrImage)
{
	...
	StopImagePlayTimerAndCleanFrameInfo(FRAME_TYPE_ScrollBar_Button2DisabledImage);//如果定时器存在,停止这个图片的定时器
    Invalidate();
}

bool CScrollBarUI::GetShowButton3()
{
	return m_bShowButton3;
}

void CScrollBarUI::SetShowButton3(bool bShow)
{
	m_bShowButton3 = bShow;
	SetPos(m_rcItem);
}

LPCTSTR CScrollBarUI::GetButton3NormalImage()
{
	return m_sButton3NormalImage;
}

void CScrollBarUI::SetButton3NormalImage(LPCTSTR pStrImage)
{
	m_sButton3NormalImage = pStrImage;
	StopImagePlayTimerAndCleanFrameInfo(FRAME_TYPE_ScrollBar_Button3NormalImage);//如果定时器存在,停止这个图片的定时器
	Invalidate();
}

LPCTSTR CScrollBarUI::GetButton3HotImage()
{
	return m_sButton3HotImage;
}

void CScrollBarUI::SetButton3HotImage(LPCTSTR pStrImage)
{
	m_sButton3HotImage = pStrImage;
	StopImagePlayTimerAndCleanFrameInfo(FRAME_TYPE_ScrollBar_Button3HotImage);//如果定时器存在,停止这个图片的定时器
	Invalidate();
}

LPCTSTR CScrollBarUI::GetButton3PushedImage()
{
	return m_sButton3PushedImage;
}

void CScrollBarUI::SetButton3PushedImage(LPCTSTR pStrImage)
{
	m_sButton3PushedImage = pStrImage;
	StopImagePlayTimerAndCleanFrameInfo(FRAME_TYPE_ScrollBar_Button3PushedImage);//如果定时器存在,停止这个图片的定时器
	Invalidate();
}

LPCTSTR CScrollBarUI::GetButton3DisabledImage()
{
	return m_sButton3DisabledImage;
}

void CScrollBarUI::SetButton3DisabledImage(LPCTSTR pStrImage)
{
	m_sButton3DisabledImage = pStrImage;
	StopImagePlayTimerAndCleanFrameInfo(FRAME_TYPE_ScrollBar_Button3DisabledImage);//如果定时器存在,停止这个图片的定时器
	Invalidate();
}
...
void CScrollBarUI::SetThumbNormalImage(LPCTSTR pStrImage)
{
	...
	StopImagePlayTimerAndCleanFrameInfo(FRAME_TYPE_ScrollBar_ThumbNormalImage);//如果定时器存在,停止这个图片的定时器
    Invalidate();
}
...
{
    m_sThumbHotImage = pStrImage;
	StopImagePlayTimerAndCleanFrameInfo(FRAME_TYPE_ScrollBar_ThumbHotImage);//如果定时器存在,停止这个图片的定时器
    Invalidate();
}
...
{
    m_sThumbPushedImage = pStrImage;
	StopImagePlayTimerAndCleanFrameInfo(FRAME_TYPE_ScrollBar_ThumbPushedImage);//如果定时器存在,停止这个图片的定时器
    Invalidate();
}
...
void CScrollBarUI::SetThumbDisabledImage(LPCTSTR pStrImage)
{
	...
	StopImagePlayTimerAndCleanFrameInfo(FRAME_TYPE_ScrollBar_ThumbDisabledImage);//如果定时器存在,停止这个图片的定时器
    Invalidate();
}
...
void CScrollBarUI::SetRailNormalImage(LPCTSTR pStrImage)
{
	...
	StopImagePlayTimerAndCleanFrameInfo(FRAME_TYPE_ScrollBar_RailNormalImage);//如果定时器存在,停止这个图片的定时器
    Invalidate();
}
...
void CScrollBarUI::SetRailHotImage(LPCTSTR pStrImage)
{
	...
	StopImagePlayTimerAndCleanFrameInfo(FRAME_TYPE_ScrollBar_RailHotImage);//如果定时器存在,停止这个图片的定时器
    Invalidate();
}
...
void CScrollBarUI::SetRailPushedImage(LPCTSTR pStrImage)
{
	...
	StopImagePlayTimerAndCleanFrameInfo(FRAME_TYPE_ScrollBar_RailPushedImage);//如果定时器存在,停止这个图片的定时器
    Invalidate();
}
...
void CScrollBarUI::SetRailDisabledImage(LPCTSTR pStrImage)
{
	...
	StopImagePlayTimerAndCleanFrameInfo(FRAME_TYPE_ScrollBar_RailDisabledImage);//如果定时器存在,停止这个图片的定时器
    Invalidate();
}
...
void CScrollBarUI::SetBkNormalImage(LPCTSTR pStrImage)
{
	...
	StopImagePlayTimerAndCleanFrameInfo(FRAME_TYPE_ScrollBar_BkNormalImage);//如果定时器存在,停止这个图片的定时器
    Invalidate();
}
...
void CScrollBarUI::SetBkHotImage(LPCTSTR pStrImage)
{
	...
	StopImagePlayTimerAndCleanFrameInfo(FRAME_TYPE_ScrollBar_BkHotImage);//如果定时器存在,停止这个图片的定时器
    Invalidate();
}
...
void CScrollBarUI::SetBkPushedImage(LPCTSTR pStrImage)
{
	...
	StopImagePlayTimerAndCleanFrameInfo(FRAME_TYPE_ScrollBar_BkPushedImage);//如果定时器存在,停止这个图片的定时器
    Invalidate();
}
...
void CScrollBarUI::SetBkDisabledImage(LPCTSTR pStrImage)
{
	...
	StopImagePlayTimerAndCleanFrameInfo(FRAME_TYPE_ScrollBar_BkDisabledImage);//如果定时器存在,停止这个图片的定时器
    Invalidate();
}
...
void CScrollBarUI::SetPos(RECT rc)
{
    ...
    if( m_bHorizontal ) {
		...
		if (m_bShowButton0) cx -= m_cxyFixed.cy;
        if( m_bShowButton1 ) cx -= m_cxyFixed.cy;
        if( m_bShowButton2 ) cx -= m_cxyFixed.cy;
		if (m_bShowButton3) cx -= m_cxyFixed.cy;
        if( cx > m_cxyFixed.cy ) {
			// button0 rect.
			m_rcButton0.left = rc.left;
			m_rcButton0.top = rc.top;
			if (m_bShowButton0) {
				m_rcButton0.right = m_rcButton0.left + m_cxyFixed.cy;
				m_rcButton0.bottom = m_rcButton0.top + m_cxyFixed.cy;
			}
			else {
				m_rcButton0.right = m_rcButton0.left;
				m_rcButton0.bottom = m_rcButton0.top;
			}
			// button1 rect.
            m_rcButton1.left = m_rcButton0.right;
            m_rcButton1.top = rc.top;
            if( m_bShowButton1 ) {
                m_rcButton1.right = m_rcButton1.left + m_cxyFixed.cy;
                m_rcButton1.bottom = m_rcButton1.top + m_cxyFixed.cy;
            }
            else {
			...
			// button3 rect.
            m_rcButton3.top = rc.top;
            m_rcButton3.right = rc.right;
            if( m_bShowButton3 ) {
                m_rcButton3.left = m_rcButton3.right - m_cxyFixed.cy;
                m_rcButton3.bottom = m_rcButton3.top + m_cxyFixed.cy;
            }
            else {
                m_rcButton3.left = m_rcButton3.right;
                m_rcButton3.bottom = m_rcButton3.top;
            }
			// button2 rect.
			m_rcButton2.top = rc.top;
			m_rcButton2.right = m_rcButton3.left;
			if (m_bShowButton2) {
				m_rcButton2.left = m_rcButton2.right - m_cxyFixed.cy;
				m_rcButton2.bottom = m_rcButton2.top + m_cxyFixed.cy;
			}
			else {
            ...
            int cxButton = (rc.right - rc.left) / 4;
            if( cxButton > m_cxyFixed.cy ) cxButton = m_cxyFixed.cy;
			//button0 rect.
            m_rcButton0.left = rc.left;
            m_rcButton0.top = rc.top;
            if( m_bShowButton0 ) {
                m_rcButton0.right = m_rcButton0.left + cxButton;
                m_rcButton0.bottom = m_rcButton0.top + m_cxyFixed.cy;
            }
            else {
                m_rcButton0.right = m_rcButton0.left;
			...
			m_rcButton1.top = rc.top;
			if( m_bShowButton1 ) {
				m_rcButton1.right = m_rcButton1.left + cxButton;
				m_rcButton1.bottom = m_rcButton1.top + m_cxyFixed.cy;
			}
			else {
				m_rcButton1.right = m_rcButton1.left;
			...
			m_rcButton3.right = rc.right;
			if( m_bShowButton3 ) {
				m_rcButton3.left = m_rcButton3.right - cxButton;
				m_rcButton3.bottom = m_rcButton3.top + m_cxyFixed.cy;
			}
			else {
				m_rcButton3.left = m_rcButton3.right;
            ...
            m_rcButton2.right = m_rcButton3.left;
            if( m_bShowButton2 ) {
                m_rcButton2.left = m_rcButton2.right - cxButton;
                m_rcButton2.bottom = m_rcButton2.top + m_cxyFixed.cy;
            }
            else {
                m_rcButton2.left = m_rcButton2.right;
		...
		if( m_bShowButton0 ) cy -= m_cxyFixed.cx;
		if( m_bShowButton1 ) cy -= m_cxyFixed.cx;
        if( m_bShowButton2 ) cy -= m_cxyFixed.cx;
        if( m_bShowButton3 ) cy -= m_cxyFixed.cx;
        if( cy > m_cxyFixed.cx ) {
			// button0 rect.
			m_rcButton0.left = rc.left;
			m_rcButton0.top = rc.top;
			if( m_bShowButton0 ) {
				m_rcButton0.right = rc.left + m_cxyFixed.cx;
				m_rcButton0.bottom = rc.top + m_cxyFixed.cx;
			}
			else {
				m_rcButton0.right = m_rcButton0.left;
				m_rcButton0.bottom = m_rcButton0.top;
            ...
            m_rcButton1.top = m_rcButton0.bottom;
            if( m_bShowButton1 ) {
                m_rcButton1.right = m_rcButton1.left + m_cxyFixed.cx;
                m_rcButton1.bottom = m_rcButton1.top + m_cxyFixed.cx;
            }
            else {
                m_rcButton1.right = m_rcButton1.left;
                m_rcButton1.bottom = m_rcButton1.top;
			...
			// button3 rect.
            m_rcButton3.left = rc.left;
            m_rcButton3.bottom = rc.bottom;
            if( m_bShowButton3 ) {
                m_rcButton3.top = m_rcButton3.bottom - m_cxyFixed.cx;
                m_rcButton3.right = m_rcButton3.left + m_cxyFixed.cx;
            }
            else {
                m_rcButton3.top = m_rcButton3.bottom;
                m_rcButton3.right = m_rcButton3.left;
            }
			// button2 rect.
			m_rcButton2.left = rc.left;
			m_rcButton2.bottom = m_rcButton3.top;
			if( m_bShowButton1 ) {
				m_rcButton2.right = m_rcButton2.left + m_cxyFixed.cx;
				m_rcButton2.top = m_rcButton2.bottom - m_cxyFixed.cx;
			}
			else {
            ...
            int cyButton = (rc.bottom - rc.top) / 4;
			...
			if( m_bShowButton0 ) {
				m_rcButton0.right = rc.left + m_cxyFixed.cx;
				m_rcButton0.bottom = rc.top + cyButton;
			}
			else {
				m_rcButton0.right = m_rcButton0.left;
				m_rcButton0.bottom = m_rcButton0.top;
			...
			if( m_bShowButton1 ) {
				m_rcButton1.right = m_rcButton1.left + m_cxyFixed.cx;
				m_rcButton1.bottom = m_rcButton1.top + cyButton;
			}
			else {
				m_rcButton1.right = m_rcButton1.left;
				m_rcButton1.bottom = m_rcButton1.top;
			...
			if( m_bShowButton3 ) {
				m_rcButton3.top = m_rcButton3.bottom - m_cxyFixed.cx;
				m_rcButton3.right = m_rcButton3.left + cyButton;
			}
			else {
				m_rcButton3.top = m_rcButton3.bottom;
				m_rcButton3.right = m_rcButton3.left;
			...
			if( m_bShowButton1 ) {
				m_rcButton2.right = m_rcButton2.left + m_cxyFixed.cx;
				m_rcButton2.top = m_rcButton2.bottom - cyButton;
			}
			else {
				m_rcButton2.right = m_rcButton2.left;
				m_rcButton2.bottom = m_rcButton2.top;
...
void CScrollBarUI::DoEvent(TEventUI& event)
{
    ...
    if( event.Type == UIEVENT_BUTTONDOWN || event.Type == UIEVENT_DBLCLICK )
    {
		...
		if (::PtInRect(&m_rcButton0, event.ptMouse)) {
			m_uButton0State |= UISTATE_PUSHED;
			if (!m_bHorizontal) {
				if (m_pOwner != NULL) m_pOwner->HomeUp();
				else SetScrollPos(0);
			}
			else {
				if (m_pOwner != NULL) m_pOwner->HomeLeft();
				else SetScrollPos(0);
			}
		}
        else if( ::PtInRect(&m_rcButton1, event.ptMouse) ) {
		...
		else if (::PtInRect(&m_rcButton3, event.ptMouse)) {
			m_uButton3State |= UISTATE_PUSHED;
			if (!m_bHorizontal) {
				if (m_pOwner != NULL) m_pOwner->EndDown();
				else SetScrollPos(0);
			}
			else {
				if (m_pOwner != NULL) m_pOwner->EndRight();
				else SetScrollPos(0);
			}
		}
    ...
    if( event.Type == UIEVENT_BUTTONUP )
    {
		...
		else if ((m_uButton0State & UISTATE_PUSHED) != 0) {
			m_uButton0State &= ~UISTATE_PUSHED;
			Invalidate();
		}
		...
		else if ((m_uButton3State & UISTATE_PUSHED) != 0) {
			m_uButton3State &= ~UISTATE_PUSHED;
			Invalidate();
		}
        return;
    ...
    {
        if( IsEnabled() ) {
			if (::PtInRect(&m_rcButton0, event.ptMouse)) m_uButton0State |= UISTATE_HOT;
            m_uButton1State |= UISTATE_HOT;
            m_uButton2State |= UISTATE_HOT;
			if (::PtInRect(&m_rcButton3, event.ptMouse)) m_uButton3State |= UISTATE_HOT;
    ...
    {
        if( IsEnabled() ) {
			m_uButton0State &= ~UISTATE_HOT;
            m_uButton1State &= ~UISTATE_HOT;
            m_uButton2State &= ~UISTATE_HOT;
			m_uButton3State &= ~UISTATE_HOT;
            m_uThumbState &= ~UISTATE_HOT;
            Invalidate();
        }
        return;
...
{
	if( _tcscmp(pstrName, _T("button0normalimage")) == 0 ) SetButton0NormalImage(pstrValue);
	else if( _tcscmp(pstrName, _T("button0hotimage")) == 0 ) SetButton0HotImage(pstrValue);
	else if( _tcscmp(pstrName, _T("button0pushedimage")) == 0 ) SetButton0PushedImage(pstrValue);
	else if( _tcscmp(pstrName, _T("button0disabledimage")) == 0 ) SetButton0DisabledImage(pstrValue);
	...
	else if( _tcscmp(pstrName, _T("button3normalimage")) == 0 ) SetButton3NormalImage(pstrValue);
	else if( _tcscmp(pstrName, _T("button3hotimage")) == 0 ) SetButton3HotImage(pstrValue);
	else if( _tcscmp(pstrName, _T("button3pushedimage")) == 0 ) SetButton3PushedImage(pstrValue);
	else if( _tcscmp(pstrName, _T("button3disabledimage")) == 0 ) SetButton3DisabledImage(pstrValue);
	...
	else if (_tcscmp(pstrName, _T("showbutton0")) == 0) SetShowButton0(_tcscmp(pstrValue, _T("true")) == 0);
	...
	else if (_tcscmp(pstrName, _T("showbutton3")) == 0) SetShowButton3(_tcscmp(pstrValue, _T("true")) == 0);
    else CControlUI::SetAttribute(pstrName, pstrValue);
}
...
void CScrollBarUI::PaintBk(HDC hDC)
{
	...
	if (IsHorizontal()) {
		rcBk.top = 0;
		rcBk.left = m_rcButton1.right - m_rcItem.left;
		rcBk.bottom = m_rcItem.bottom - m_rcItem.top;
		rcBk.right = m_rcButton2.left - m_rcItem.left;
	}
	else {
		rcBk.left = 0;
		rcBk.top = m_rcButton1.bottom - m_rcItem.top;
		rcBk.right = m_rcItem.right - m_rcItem.left;
		rcBk.bottom = m_rcButton2.top - m_rcItem.top;
	}
	m_sImageModify.SmallFormat(_T("dest='%d,%d,%d,%d'"), rcBk.left, rcBk.top, rcBk.right, rcBk.bottom);

    if( (m_uThumbState & UISTATE_DISABLED) != 0 ) {
        if( !m_sBkDisabledImage.IsEmpty() ) {
            if( !DrawImage(hDC, (LPCTSTR)m_sBkDisabledImage, (LPCTSTR)m_sImageModify, FRAME_TYPE_ScrollBar_BkDisabledImage)) m_sBkDisabledImage.Empty();
            else return;
        }
    }
    else if( (m_uThumbState & UISTATE_PUSHED) != 0 ) {
        if( !m_sBkPushedImage.IsEmpty() ) {
            if( !DrawImage(hDC, (LPCTSTR)m_sBkPushedImage, (LPCTSTR)m_sImageModify , FRAME_TYPE_ScrollBar_BkPushedImage)) m_sBkPushedImage.Empty();
            else return;
        }
    }
    else if( (m_uThumbState & UISTATE_HOT) != 0 ) {
        if( !m_sBkHotImage.IsEmpty() ) {
            if( !DrawImage(hDC, (LPCTSTR)m_sBkHotImage, (LPCTSTR)m_sImageModify , FRAME_TYPE_ScrollBar_BkHotImage)) m_sBkHotImage.Empty();
            else return;
        }
    }

    if( !m_sBkNormalImage.IsEmpty() ) {
        if( !DrawImage(hDC, (LPCTSTR)m_sBkNormalImage, (LPCTSTR)m_sImageModify , FRAME_TYPE_ScrollBar_BkNormalImage)) m_sBkNormalImage.Empty();
        else return;
    }
}

void CScrollBarUI::PaintButton0(HDC hDC)
{
	if( !m_bShowButton0 ) return;

	if( !IsEnabled() ) m_uButton0State |= UISTATE_DISABLED;
	else m_uButton0State &= ~ UISTATE_DISABLED;

	m_sImageModify.Empty();
	m_sImageModify.SmallFormat(_T("dest='%d,%d,%d,%d'"), m_rcButton0.left - m_rcItem.left, \
		m_rcButton0.top - m_rcItem.top, m_rcButton0.right - m_rcItem.left, m_rcButton0.bottom - m_rcItem.top);

	if( (m_uButton0State & UISTATE_DISABLED) != 0 ) {
		if( !m_sButton0DisabledImage.IsEmpty() ) {
			if( !DrawImage(hDC, (LPCTSTR)m_sButton0DisabledImage, (LPCTSTR)m_sImageModify, FRAME_TYPE_ScrollBar_Button0DisabledImage) ) m_sButton0DisabledImage.Empty();
			else return;
		}
	}
	else if( (m_uButton0State & UISTATE_PUSHED) != 0 ) {
		if( !m_sButton0PushedImage.IsEmpty() ) {
			if( !DrawImage(hDC, (LPCTSTR)m_sButton0PushedImage, (LPCTSTR)m_sImageModify, FRAME_TYPE_ScrollBar_Button0PushedImage) ) m_sButton0PushedImage.Empty();
			else return;
		}
	}
	else if( (m_uButton0State & UISTATE_HOT) != 0 ) {
		if( !m_sButton0HotImage.IsEmpty() ) {
			if( !DrawImage(hDC, (LPCTSTR)m_sButton0HotImage, (LPCTSTR)m_sImageModify, FRAME_TYPE_ScrollBar_Button0HotImage) ) m_sButton0HotImage.Empty();
			else return;
		}
	}

	if( !m_sButton0NormalImage.IsEmpty() ) {
		if( !DrawImage(hDC, (LPCTSTR)m_sButton0NormalImage, (LPCTSTR)m_sImageModify, FRAME_TYPE_ScrollBar_Button0NormalImage) ) m_sButton0NormalImage.Empty();
		else return;
	}

	DWORD dwBorderColor = 0xFF85E4FF;
	int nBorderSize = 2;
	CRenderEngine::DrawRect(hDC, m_rcButton0, nBorderSize, dwBorderColor);
}
...
void CScrollBarUI::PaintButton1(HDC hDC)
{
    ...
    if( (m_uButton1State & UISTATE_DISABLED) != 0 ) {
        if( !m_sButton1DisabledImage.IsEmpty() ) {
            if( !DrawImage(hDC, (LPCTSTR)m_sButton1DisabledImage, (LPCTSTR)m_sImageModify, FRAME_TYPE_ScrollBar_Button1DisabledImage) ) m_sButton1DisabledImage.Empty();
            else return;
        }
    }
    else if( (m_uButton1State & UISTATE_PUSHED) != 0 ) {
        if( !m_sButton1PushedImage.IsEmpty() ) {
            if( !DrawImage(hDC, (LPCTSTR)m_sButton1PushedImage, (LPCTSTR)m_sImageModify, FRAME_TYPE_ScrollBar_Button1PushedImage) ) m_sButton1PushedImage.Empty();
            else return;
        }
    }
    else if( (m_uButton1State & UISTATE_HOT) != 0 ) {
        if( !m_sButton1HotImage.IsEmpty() ) {
            if( !DrawImage(hDC, (LPCTSTR)m_sButton1HotImage, (LPCTSTR)m_sImageModify, FRAME_TYPE_ScrollBar_Button1HotImage) ) m_sButton1HotImage.Empty();
            else return;
        }
    }

    if( !m_sButton1NormalImage.IsEmpty() ) {
        if( !DrawImage(hDC, (LPCTSTR)m_sButton1NormalImage, (LPCTSTR)m_sImageModify, FRAME_TYPE_ScrollBar_Button1NormalImage) ) m_sButton1NormalImage.Empty();
        else return;
    }
...
void CScrollBarUI::PaintButton2(HDC hDC)
{
    ...
    if( (m_uButton2State & UISTATE_DISABLED) != 0 ) {
        if( !m_sButton2DisabledImage.IsEmpty() ) {
            if( !DrawImage(hDC, (LPCTSTR)m_sButton2DisabledImage, (LPCTSTR)m_sImageModify, FRAME_TYPE_ScrollBar_Button2DisabledImage) ) m_sButton2DisabledImage.Empty();
            else return;
        }
    }
    else if( (m_uButton2State & UISTATE_PUSHED) != 0 ) {
        if( !m_sButton2PushedImage.IsEmpty() ) {
            if( !DrawImage(hDC, (LPCTSTR)m_sButton2PushedImage, (LPCTSTR)m_sImageModify, FRAME_TYPE_ScrollBar_Button2PushedImage) ) m_sButton2PushedImage.Empty();
            else return;
        }
    }
    else if( (m_uButton2State & UISTATE_HOT) != 0 ) {
        if( !m_sButton2HotImage.IsEmpty() ) {
            if( !DrawImage(hDC, (LPCTSTR)m_sButton2HotImage, (LPCTSTR)m_sImageModify, FRAME_TYPE_ScrollBar_Button2HotImage) ) m_sButton2HotImage.Empty();
            else return;
        }
    }

    if( !m_sButton2NormalImage.IsEmpty() ) {
        if( !DrawImage(hDC, (LPCTSTR)m_sButton2NormalImage, (LPCTSTR)m_sImageModify, FRAME_TYPE_ScrollBar_Button2NormalImage) ) m_sButton2NormalImage.Empty();
        else return;
    }
...
void CScrollBarUI::PaintButton3(HDC hDC)
{
	if( !m_bShowButton3 ) return;

	if( !IsEnabled() ) m_uButton3State |= UISTATE_DISABLED;
	else m_uButton3State &= ~ UISTATE_DISABLED;

	m_sImageModify.Empty();
	m_sImageModify.SmallFormat(_T("dest='%d,%d,%d,%d'"), m_rcButton3.left - m_rcItem.left, \
		m_rcButton3.top - m_rcItem.top, m_rcButton3.right - m_rcItem.left, m_rcButton3.bottom - m_rcItem.top);

	if( (m_uButton3State & UISTATE_DISABLED) != 0 ) {
		if( !m_sButton3DisabledImage.IsEmpty() ) {
			if( !DrawImage(hDC, (LPCTSTR)m_sButton3DisabledImage, (LPCTSTR)m_sImageModify, FRAME_TYPE_ScrollBar_Button3DisabledImage) ) m_sButton3DisabledImage.Empty();
			else return;
		}
	}
	else if( (m_uButton3State & UISTATE_PUSHED) != 0 ) {
		if( !m_sButton3PushedImage.IsEmpty() ) {
			if( !DrawImage(hDC, (LPCTSTR)m_sButton3PushedImage, (LPCTSTR)m_sImageModify, FRAME_TYPE_ScrollBar_Button3PushedImage) ) m_sButton3PushedImage.Empty();
			else return;
		}
	}
	else if( (m_uButton3State & UISTATE_HOT) != 0 ) {
		if( !m_sButton3HotImage.IsEmpty() ) {
			if( !DrawImage(hDC, (LPCTSTR)m_sButton3HotImage, (LPCTSTR)m_sImageModify, FRAME_TYPE_ScrollBar_Button3HotImage) ) m_sButton3HotImage.Empty();
			else return;
		}
	}

	if( !m_sButton3NormalImage.IsEmpty() ) {
		if( !DrawImage(hDC, (LPCTSTR)m_sButton3NormalImage, (LPCTSTR)m_sImageModify, FRAME_TYPE_ScrollBar_Button3NormalImage) ) m_sButton3NormalImage.Empty();
		else return;
	}

	DWORD dwBorderColor = 0xFF85E4FF;
	int nBorderSize = 2;
	CRenderEngine::DrawRect(hDC, m_rcButton3, nBorderSize, dwBorderColor);
}
...
void CScrollBarUI::PaintThumb(HDC hDC)
{
    ...
    if( (m_uThumbState & UISTATE_DISABLED) != 0 ) {
        if( !m_sThumbDisabledImage.IsEmpty() ) {
            if( !DrawImage(hDC, (LPCTSTR)m_sThumbDisabledImage, (LPCTSTR)m_sImageModify, FRAME_TYPE_ScrollBar_ThumbDisabledImage) ) m_sThumbDisabledImage.Empty();
            else return;
        }
    }
    else if( (m_uThumbState & UISTATE_PUSHED) != 0 ) {
        if( !m_sThumbPushedImage.IsEmpty() ) {
            if( !DrawImage(hDC, (LPCTSTR)m_sThumbPushedImage, (LPCTSTR)m_sImageModify, FRAME_TYPE_ScrollBar_ThumbPushedImage) ) m_sThumbPushedImage.Empty();
            else return;
        }
    }
    else if( (m_uThumbState & UISTATE_HOT) != 0 ) {
        if( !m_sThumbHotImage.IsEmpty() ) {
            if( !DrawImage(hDC, (LPCTSTR)m_sThumbHotImage, (LPCTSTR)m_sImageModify, FRAME_TYPE_ScrollBar_ThumbHotImage) ) m_sThumbHotImage.Empty();
            else return;
        }
    }

    if( !m_sThumbNormalImage.IsEmpty() ) {
        if( !DrawImage(hDC, (LPCTSTR)m_sThumbNormalImage, (LPCTSTR)m_sImageModify, FRAME_TYPE_ScrollBar_ThumbNormalImage) ) m_sThumbNormalImage.Empty();
        else return;
    }
...
void CScrollBarUI::PaintRail(HDC hDC)
{
    ...
    if( (m_uThumbState & UISTATE_DISABLED) != 0 ) {
        if( !m_sRailDisabledImage.IsEmpty() ) {
            if( !DrawImage(hDC, (LPCTSTR)m_sRailDisabledImage, (LPCTSTR)m_sImageModify, FRAME_TYPE_ScrollBar_RailDisabledImage) ) m_sRailDisabledImage.Empty();
            else return;
        }
    }
    else if( (m_uThumbState & UISTATE_PUSHED) != 0 ) {
        if( !m_sRailPushedImage.IsEmpty() ) {
            if( !DrawImage(hDC, (LPCTSTR)m_sRailPushedImage, (LPCTSTR)m_sImageModify, FRAME_TYPE_ScrollBar_RailPushedImage) ) m_sRailPushedImage.Empty();
            else return;
        }
    }
    else if( (m_uThumbState & UISTATE_HOT) != 0 ) {
        if( !m_sRailHotImage.IsEmpty() ) {
            if( !DrawImage(hDC, (LPCTSTR)m_sRailHotImage, (LPCTSTR)m_sImageModify, FRAME_TYPE_ScrollBar_RailHotImage) ) m_sRailHotImage.Empty();
            else return;
        }
    }

    if( !m_sRailNormalImage.IsEmpty() ) {
        if( !DrawImage(hDC, (LPCTSTR)m_sRailNormalImage, (LPCTSTR)m_sImageModify, FRAME_TYPE_ScrollBar_RailNormalImage) ) m_sRailNormalImage.Empty();
        else return;
    }
}
