...
CSerialization::CSerialization( std::string &strBuffer )
{
	m_strBuffer = strBuffer;
	m_iSerializationType = enum_Serialization_Type_Default;
	m_iBufferCreateType = enum_Buffer_Create_Type_OutSide;
	m_pBuffer = (char*)m_strBuffer.data();
	m_dwBufferLen = m_strBuffer.size();
	m_dwDataLen = 0;
}
...
void CSerialization::Serialization(string& strValue)
{
	...
	DWORD dwDataLen = static_cast<DWORD>(strValue.size());
	Serialization(dwDataLen);
	if (m_dwBufferLen < (m_dwDataLen + dwDataLen))
	{
		if (m_iBufferCreateType == enum_Buffer_Create_Type_OutSide)	// 外部分配的缓存
		{
...
void CSerialization::ToBuffer( std::string &strBuffer )
{
	strBuffer.clear();
	strBuffer.append(m_pBuffer, m_dwDataLen);
}
