...
void GetFriendlyNames(REFGUID category, std::vector<std::wstring>& friendlyNames)
{
	...
	while (pEnum->Next(1, &pMoniker, nullptr) == S_OK && pMoniker)
	{
...
HRESULT BindFilter(REFGUID category, LPCTSTR lpFriendlyName, IBaseFilter ** ppFilter)
{
	...
	pEnum->Reset();
	for (CComPtr<IMoniker> pMoniker;pEnum->Next(1, &pMoniker, nullptr) == S_OK && pMoniker; pMoniker.Release())
	{
		CComPtr<IPropertyBag> pPropBag;
		HRESULT hr = pMoniker->BindToStorage(0, 0, IID_PPV_ARGS(&pPropBag));
		if (FAILED(hr))
...
HRESULT GetPin(IBaseFilter * pFilter, PIN_DIRECTION dirrequired, int iNum, IPin ** ppPin)
{
	...
	do
	{
		...
		*ppPin = nullptr;
		...
		pEnumPins->Reset();
		IPin *pPin = nullptr;
		while (pEnumPins->Next(1, &pPin, nullptr) == S_OK && pPin)
		{
...
HRESULT DisconnectFilter(IGraphBuilder * pGraph, IBaseFilter * pFilter)
{
	HRESULT hr = S_FALSE;
	...
	do
	{
		...
		pEnumPins->Reset();
		ULONG cFetched = 0;
		for (CComPtr<IPin> pPin; pEnumPins->Next(1, &pPin, &cFetched) == S_OK && pPin; pPin.Release())
		{
