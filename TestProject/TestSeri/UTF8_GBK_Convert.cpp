...
////////////////////////////////////////////////////////////////////////////////////////////////////
CUTF8_GBK_Convert::CUTF8_GBK_Convert()
{
	m_bInit = false;
	m_pGBKToUTF16_2 = NULL;
	m_GBKToUTF16_2_Size = 0;
	m_pGBKToUTF16_3 = NULL;
	m_GBKToUTF16_3_Size = 0;
	memset(m_GBKToUTF16, 0, sizeof(m_GBKToUTF16));
	memset(m_UTF16ToGBK, 0, sizeof(m_UTF16ToGBK));
}
...
// 是否初始化
bool CUTF8_GBK_Convert::IsOpen()
{
	return m_bInit;
}

// 将某字符集的字符串转换为另一个字符集的字符串
int CUTF8_GBK_Convert::ConvertCode(int aiCharSetConvertType, const char* apInput, unsigned long alInputSize, char* apOutput, unsigned long& alOutputSize)
{
	// 返回值
	int iReturn = -1;

	switch (aiCharSetConvertType)
	{
	case CHARSET_CONVERT_TYPE_GBK_TO_UTF8:	// GBK转UTF8
		iReturn = GBKToUTF8(apInput, alInputSize, apOutput, alOutputSize);
		break;
	case CHARSET_CONVERT_TYPE_UTF8_TO_GBK:	// UTF8转GBK
		iReturn = UTF8ToGBK(apInput, alInputSize, apOutput, alOutputSize);
		break;
	default:
		assert(false);
		break;
...
// 初始化GBK到UTF8的2字节字符
void CUTF8_GBK_Convert::InitGBKToUTF16_2()
{
	static const unsigned short GBKToUTF16_2[] =
	{
		0x8000, 0x20AC,	// 对应"€" {{{
		0x8140, 0x4E02,
		0x8144, 0x4E0F,
	...
	m_GBKToUTF16_2_Size = sizeof(GBKToUTF16_2) / sizeof(short);
	m_pGBKToUTF16_2 = GBKToUTF16_2;
}

// 初始化GBK到UTF8的3字节字符
void CUTF8_GBK_Convert::InitGBKToUTF16_3()
{
	static const unsigned short GBKToUTF16_3[] =
	{
	...
	m_GBKToUTF16_3_Size = sizeof(GBKToUTF16_3) / sizeof(short);
	m_pGBKToUTF16_3 = GBKToUTF16_3;
}

// 初始化GBK到UTF16的字符
void CUTF8_GBK_Convert::InitGBKToUTF16()
{
	unsigned short c = 0;
	unsigned short d = 0;

	for (c = 0; c < m_GBKToUTF16_2_Size; c += 2)
	{
		m_GBKToUTF16[m_pGBKToUTF16_2[c] - 0x8000] = m_pGBKToUTF16_2[c + 1];
	}

	for (c = 0; c < m_GBKToUTF16_3_Size; c += 3)
	{
		for (d = m_pGBKToUTF16_3[c]; d <= m_pGBKToUTF16_3[c + 1]; d++)
		{
			m_GBKToUTF16[d - 0x8000] = m_pGBKToUTF16_3[c + 2] + d - m_pGBKToUTF16_3[c];
		}
	}
}

// 初始化UTF16到GBK的字符
void CUTF8_GBK_Convert::InitUTF16ToGBK()
{
	if (m_pGBKToUTF16_2 == NULL || m_GBKToUTF16_2_Size == 0 || m_pGBKToUTF16_3 == NULL || m_GBKToUTF16_3_Size == 0)
	{
		assert(false);
		return;
	}
	...
	for (c = 0; c <  m_GBKToUTF16_2_Size; c += 2)
	{
		m_UTF16ToGBK[m_pGBKToUTF16_2[c + 1]] = m_pGBKToUTF16_2[c];
	}
	...
	for (c = 0; c < m_GBKToUTF16_3_Size; c += 3)
	{
		for (d = m_pGBKToUTF16_3[c]; d <= m_pGBKToUTF16_3[c + 1]; d++)
		{
			m_UTF16ToGBK[m_pGBKToUTF16_3[c + 2] + d - m_pGBKToUTF16_3[c]] = d;
		}
	}
}

// GBK转UTF8
int CUTF8_GBK_Convert::GBKToUTF8(const char* apInput, unsigned long alInputSize, char* apOutput, unsigned long& alOutputSize)
{
	if (apInput == NULL || apOutput == NULL)
	{
		assert(false);
		return -1;
	}

	if (alInputSize == 0 || alOutputSize == 0 || alInputSize > alOutputSize)
	{
		return 0;
	}

	unsigned long lInputIndex = 0;
	unsigned long lOutputIndex = 0;
	int iFlag = 0;

	for (lInputIndex = 0; lInputIndex < alInputSize; lInputIndex++)
	{
		if (iFlag != 0)
		{
			iFlag = 0;
			unsigned short sTemp = m_GBKToUTF16[COMPBYTE((signed char)apInput[lInputIndex - 1], (signed char)apInput[lInputIndex]) & ~0x8000];
			if (sTemp == 0)
			{
				continue;
			}
			...
			{
				apOutput[lOutputIndex++] = 0xE0 | (sTemp >> 12);
				apOutput[lOutputIndex++] = 0x80 | ((sTemp >> 6) & 0x3F);
				apOutput[lOutputIndex++] = 0x80 | (sTemp & 0x3F);
			}
			else if (sTemp >= 0x80)
			{
				apOutput[lOutputIndex++] = 0xC0 | (sTemp >> 6);
				apOutput[lOutputIndex++] = 0x80 | (sTemp & 0x3F);
			}
			else
			{
				apOutput[lOutputIndex++] = (char)sTemp;
			}
		}
		else if ((signed char)apInput[lInputIndex] < 0)
		{
			iFlag = 1;
		}
		else
		{
			apOutput[lOutputIndex++] = apInput[lInputIndex];
		}

		if (lOutputIndex >= alOutputSize)
		{
			break;
		}
	}

	apOutput[lOutputIndex] = 0;
	alOutputSize = lOutputIndex;

	return 0;
}

// UTF8转GBK
int CUTF8_GBK_Convert::UTF8ToGBK(const char* apInput, unsigned long alInputSize, char* apOutput, unsigned long& alOutputSize)
{
	if (apInput == NULL || apOutput == NULL)
	{
		assert(false);
	...
	for (lInputIndex = 0; lInputIndex < alInputSize; )
	{
		if ((unsigned char)apInput[lInputIndex] < 0x80)
		{
			apOutput[lOutputIndex++] = apInput[lInputIndex++];
		}
		else if ((unsigned char)apInput[lInputIndex] < 0xC2)
		{
			lInputIndex++;
		}
		else if ((unsigned char)apInput[lInputIndex] < 0xE0)
		{
			if (lInputIndex >= alInputSize - 1)
				break;

			unsigned short sTemp = m_UTF16ToGBK[((apInput[lInputIndex] & 0x1F) << 6) | (apInput[lInputIndex + 1] & 0x3F)];
			if (sTemp)
			{
				apOutput[lOutputIndex++] = sTemp >> 8;
				apOutput[lOutputIndex++] = sTemp & 0xFF;
			}

			lInputIndex += 2;
		}
		else if ((unsigned char)apInput[lInputIndex] < 0xF0)
		{
			if (lInputIndex >= alInputSize - 2)
				break;

			unsigned short sTemp = m_UTF16ToGBK[((apInput[lInputIndex] & 0x0F) << 12) | ((apInput[lInputIndex + 1] & 0x3F) << 6) | (apInput[lInputIndex + 2] & 0x3F)];
			if (sTemp)
			{
				apOutput[lOutputIndex++] = sTemp >> 8;
				apOutput[lOutputIndex++] = sTemp & 0xFF;
